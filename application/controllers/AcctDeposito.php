<?php
	Class AcctDeposito extends CI_Controller{
		...
		public function reset_data(){
			...
			redirect('AcctDeposito/addAcctDeposito');
		}
		...
		public function processAddAcctDeposito(){
			...
			$data = array(
				...
				'deposito_profit_sharing'	=> $this->input->post('deposito_profit_sharing', true),
			...
			$this->form_validation->set_rules('deposito_code', 'Kode Berjangka', 'required');
			$this->form_validation->set_rules('deposito_name', 'Nama Berjangka', 'required');
			...
			if($this->form_validation->run()==true){
				if($this->AcctDeposito_model->insertAcctDeposito($data)){
					...
					$msg = "<div class='alert alert-success alert-dismissable'>  
							<button type='button' class='close' data-dismiss='alert' aria-hidden='true'></button>					
								Tambah Data Kode Berjangka Sukses
							</div> ";
					...
					$msg = "<div class='alert alert-danger alert-dismissable'>
							<button type='button' class='close' data-dismiss='alert' aria-hidden='true'></button>					
								Tambah Data Kode Berjangka Tidak Berhasil
							</div> ";
					$this->session->set_userdata('message',$msg);
					redirect('AcctDeposito/addAcctDeposito');
				}
		...
		public function processEditAcctDeposito(){
			...
			if($this->form_validation->run()==true){
				if($this->AcctDeposito_model->updateAcctDeposito($data)){
					...
					$msg = "<div class='alert alert-success alert-dismissable'>  
							<button type='button' class='close' data-dismiss='alert' aria-hidden='true'></button>
								Edit Kode Berjangka Sukses
							</div> ";
					$this->session->set_userdata('message',$msg);
					redirect('AcctDeposito/editAcctDeposito/'.$data['deposito_id']);
				}else{
					$msg = "<div class='alert alert-danger alert-dismissable'> 
							<button type='button' class='close' data-dismiss='alert' aria-hidden='true'></button>
								Edit Kode Berjangka Tidak Berhasil
							</div> ";
					$this->session->set_userdata('message',$msg);
					redirect('AcctDeposito/editAcctDeposito/'.$data['deposito_id']);
				}
		...
		public function deleteAcctDeposito(){
			if($this->AcctDeposito_model->deleteAcctDeposito($this->uri->segment(3))){
				...
				$msg = "<div class='alert alert-success alert-dismissable'>                 
							Hapus Data Kode Berjangka Sukses
						</div> ";
				$this->session->set_userdata('message',$msg);
				redirect('AcctDeposito');
			}else{
				$msg = "<div class='alert alert-danger alert-dismissable'>                
							Hapus Data Kode Berjangka Tidak Berhasil
						</div> ";
				$this->session->set_userdata('message',$msg);
				redirect('AcctDeposito');
			}
