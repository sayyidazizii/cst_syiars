<?php
	...
	Class AcctSavingsAutoDebet extends CI_Controller{
		public function __construct(){
			parent::__construct();
			$this->load->model('Connection_model');
			$this->load->model('MainPage_model');
			$this->load->model('AcctSavingsAutoDebet_model');
		...
		public function index(){
			...
			$data['main_view']['acctsavings']		= create_double($this->AcctSavingsAutoDebet_model->getAcctSavings(), 'savings_id', 'savings_name');
			$data['main_view']['content']			= 'AcctSavingsAutoDebet/ListAcctSavingsAutoDebet_view';
			$this->load->view('MainPage_view',$data);
		}

		public function getAcctSavingsAccount_Member(){
			$auth 					 = $this->session->userdata('auth');
			$savings_auto_debet_token = md5(rand());
			$this->session->set_userdata('savings_auto_debet_token',$savings_auto_debet_token);
			...
			$corememberautodebet 	 		=  $this->AcctSavingsAutoDebet_model->getCoreMemberAutoDebet();
			$coremembernonautodebet1 		=  $this->AcctSavingsAutoDebet_model->getCoreMemberNonAutoDebet1();
			$coremembernonautodebet2 		=  $this->AcctSavingsAutoDebet_model->getCoreMemberNonAutoDebet2();
			$coremembernonautodebet_merge  	=  array_merge($coremembernonautodebet1, $coremembernonautodebet2);

			$tempArr 							= array_unique(array_column($coremembernonautodebet_merge, 'member_id'));
			$coremembernonautodebet_distinct 	= array_intersect_key($coremembernonautodebet_merge, $tempArr);
			$tempArray							= array_column($corememberautodebet, 'member_id');
			$coremembernonautodebet 			= array_intersect_key($coremembernonautodebet_distinct, $tempArray);

			$data['main_view']['corememberautodebet']		= $corememberautodebet;
			if($corememberautodebet){
				$data['main_view']['coremembernonautodebet']	= $coremembernonautodebet;
			}else{
				$data['main_view']['coremembernonautodebet']	= $coremembernonautodebet_distinct;
			}
			$data['main_view']['content']					= 'AcctSavingsAutoDebet/ListAcctSavingsAutoDebet_view';
			$this->load->view('MainPage_view',$data);
		...
		public function processAcctSavingsAutoDebet(){
			$auth 						= $this->session->userdata('auth');
			$savings_auto_debet_token	= $this->session->userdata('savings_auto_debet_token');

			$corememberautodebet 		= $this->AcctSavingsAutoDebet_model->getCoreMemberAutoDebet();
			$preferencecompany 			= $this->AcctSavingsAutoDebet_model->getPreferenceCompany();

			foreach($corememberautodebet as $key => $val){
				$data_autodebet 	= array (
					'member_id'					=> $val['member_id'],
					'savings_account_id'		=> $val['savings_account_id'],
					'amount'					=> $preferencecompany['auto_debet_amount'],
					'created_id'				=> $auth['user_id'],
					'created_on'				=> date('Y-m-d'),
					'savings_auto_debet_token'	=> $savings_auto_debet_token.$val['member_id'],
				);

				$savings_auto_debet_token = $this->AcctSavingsAutoDebet_model->getAcctSavingsAutoDebetToken($data_autodebet['savings_auto_debet_token']);

				if($savings_auto_debet_token == 0){
					if($this->AcctSavingsAutoDebet_model->insertAcctSavingsAutoDebet($data_autodebet)){
						$data_detail = array (
							'branch_id'						=> $auth['branch_id'],
							'member_id'						=> $data_autodebet['member_id'],
							'mutation_id'					=> 0,
							'transaction_date'				=> date('Y-m-d'),
							'principal_savings_amount'		=> $preferencecompany['auto_debet_amount'],
							'special_savings_amount'		=> 0,
							'mandatory_savings_amount'		=> 0,
							'operated_name'					=> $auth['username'],
							'savings_member_detail_token'	=> $data_autodebet['savings_auto_debet_token'],
						);

						if($this->AcctSavingsAutoDebet_model->insertAcctSavingsMemberDetail($data_detail)){
							$transaction_module_code 	= "AGT";

							$transaction_module_id 		= $this->AcctSavingsAutoDebet_model->getTransactionModuleID($transaction_module_code);
							$coremember 				= $this->AcctSavingsAutoDebet_model->getCoreMember_Detail($data_autodebet['member_id']);
							...
							$data_journal = array(
								...
								'journal_voucher_title'			=> 'MUTASI ANGGOTA AUTO DEBET '.$coremember['member_name'].' '.$coremember['member_no'],
								'journal_voucher_description'	=> 'MUTASI ANGGOTA AUTO DEBET '.$coremember['member_name'].' '.$coremember['member_no'],
								'journal_voucher_token'			=> $data_autodebet['savings_auto_debet_token'].$auth['branch_id'],
							...
							$this->AcctSavingsAutoDebet_model->insertAcctJournalVoucher($data_journal);

							$journal_voucher_id = $this->AcctSavingsAutoDebet_model->getJournalVoucherID($auth['user_id']);

							$account_id_default_status = $this->AcctSavingsAutoDebet_model->getAccountIDDefaultStatus($val['account_id']);

							$data_debet = array (
								'journal_voucher_id'			=> $journal_voucher_id,
								'account_id'					=> $val['account_id'],
								'journal_voucher_description'	=> 'SETORAN AUTO DEBET '.$coremember['member_name'].' '.$coremember['member_no'],
								'journal_voucher_amount'		=> $data_autodebet['amount'],
								'journal_voucher_debit_amount'	=> $data_autodebet['amount'],
								'account_id_default_status'		=> $account_id_default_status,
								'account_id_status'				=> 0,
								'journal_voucher_item_token'	=> $data_autodebet['savings_auto_debet_token'].$val['account_id'],
							);

							$this->AcctSavingsAutoDebet_model->insertAcctJournalVoucherItem($data_debet);

							if($coremember['branch_id'] != 1){
								$account_id_default_status 		= $this->AcctSavingsAutoDebet_model->getAccountIDDefaultStatus($preferencecompany['account_central_capital_id']);
								...
								$data_credit =array(
									...
									'journal_voucher_description'	=> 'SETORAN AUTO DEBET '.$coremember['member_name'].' '.$coremember['member_no'],
									'journal_voucher_amount'		=> $data_autodebet['amount'],
									'journal_voucher_credit_amount'	=> $data_autodebet['amount'],
									'account_id_status'				=> 1,
									'journal_voucher_item_token'	=> $data_autodebet['savings_auto_debet_token'].$preferencecompany['account_central_capital_id'],
								);

								$this->AcctSavingsAutoDebet_model->insertAcctJournalVoucherItem($data_credit);
							}else{
								$account_id = $this->CoreMember_model->getAccountID($preferencecompany['principal_savings_id']);

								$account_id_default_status 		= $this->AcctSavingsAutoDebet_model->getAccountIDDefaultStatus($account_id);
								...
								$data_credit =array(
									...
									'journal_voucher_credit_amount'	=> $data_autodebet['amount'],
									'account_id_status'				=> 1,
									'journal_voucher_item_token'	=> $data_autodebet['savings_auto_debet_token'].$account_id,
								);
						...
						redirect('AcctSavingsAutoDebet');
					}
				}
			}
			$this->session->unset_userdata('savings_auto_debet_token');
		...
		public function processAddAcctSavingsAutoDebet(){
			$auth = $this->session->userdata('auth');
			...
			if($this->form_validation->run()==true){
				...
				$data_log_step1 = $this->AcctSavingsAutoDebet_model->getDataLogStep1($log_step1);

				if(empty($data_log_step1)){
					$this->AcctSavingsAutoDebet_model->insertDataLogStep1($log_step1, $file);		
				} else {
					if($data_log_step1['status_process'] == 0){
						$this->AcctSavingsAutoDebet_model->createTable($log_step1, $file);
					}
				}
				...
				$data_log_step2 = $this->AcctSavingsAutoDebet_model->getDataLogStep2($log_step2);
				
				if(empty($data_log_step2)){

					$acctsavingsaccountforsrh 		= $this->AcctSavingsAutoDebet_model->getAcctSavingsAccountfoSRH($auth['branch_id']);
					...
					if(!empty($acctsavingsaccountforsrh)){
						foreach ($acctsavingsaccountforsrh as $key => $val) {
							// if($val['savings_account_daily_average_balance'] == 0){
								$yesterday_transaction_date = $this->AcctSavingsAutoDebet_model->getYesterdayTransactionDate($val['savings_account_id']);

								$last_balance_SRH 			= $this->AcctSavingsAutoDebet_model->getLastBalanceSRH($val['savings_account_id']);
								...
								$daily_average_balance_total = $this->AcctSavingsAutoDebet_model->getDailyAverageBalanceTotal($val['savings_account_id'], $data['month_period'], $data['year_period']);
					...
					if($this->AcctSavingsAutoDebet_model->insertDataLogStep($log_step2)){
						if($this->AcctSavingsAutoDebet_model->insertAcctSavingsAccountDetail($dataacctsavingsaccountdetail, $log_step2)){
							$this->AcctSavingsAutoDebet_model->insertAcctSavingsAccountTemp($data_savings);
						}
					}
				}
				...
				// $data_log_step3 = $this->AcctSavingsAutoDebet_model->getDataLogStep3($log_step3);	

				// if(empty($data_log_step3)){	
				// 	if($this->AcctSavingsAutoDebet_model->insertDataLogStep($log_step3)){
				// 		$preferencecompany 			= $this->AcctSavingsAutoDebet_model->getPreferenceCompany();

				// 		if($auth['branch_id'] == 2){
				// 			$daily_average_balance_accumulation = $this->AcctSavingsAutoDebet_model->getDailyAverageBalanceAccumulation($auth['branch_id']);

				// 			$savings_last_balance_accumulation 	= $this->AcctSavingsAutoDebet_model->getSavingsLastBalanceAccumulation($data['month_period'], $data['year_period'], $auth['branch_id']);

				// 			$deposito_last_balance_accumulation	= $this->AcctSavingsAutoDebet_model->getDepositoLastBalanceAccumulation($auth['branch_id']);
				...
				// 				$daily_avreage_balance 	= $this->AcctSavingsAutoDebet_model->getDailyAverageBalance_Savings($valS['savings_id'], $auth['branch_id']);
				...
				// 				$daily_avreage_balance 		= $this->AcctSavingsAutoDebet_model->getDepositoLastBalance_Deposito($valD['deposito_id'], $auth['branch_id']);
				...
				$data_log_step4 = $this->AcctSavingsAutoDebet_model->getDataLogStep4($log_step4);	

				if(empty($data_log_step4)){	
					$savings_daily_average_balance_minimum 	= $data['srh_minimal'];
					$acctsavingsaccount 					= $this->AcctSavingsAutoDebet_model->getAcctSavingsAccountforBasil($auth['branch_id'], $savings_daily_average_balance_minimum);
					$profitsharing 							= $this->AcctSavingsAutoDebet_model->getPreferenceProfitSharing();

					$no = 1;
					foreach ($acctsavingsaccount as $k => $v) {
						// $savings_index_amount 					= $this->AcctSavingsAutoDebet_model->getSavingsIndexAmount($v['savings_id'], $savings_profit_sharing_period);

						$savings_account_daily_average_balance 	= $this->AcctSavingsAutoDebet_model->getSavingsAccountDailyAverage($v['savings_account_id']);
					...
					if($this->AcctSavingsAutoDebet_model->insertDataLogStep($log_step4)){
						$this->AcctSavingsAutoDebet_model->insertAcctSavingsAutoDebetTemp($dataacctsavingsprofitsharing, $log_step4);
					}
				}


					redirect('AcctSavingsAutoDebet/listdata');
			} else {
		...
		public function listdata(){
			...
			$data['main_view']['acctsavingsprofitsharingtemp']		= $this->AcctSavingsAutoDebet_model->getAcctSavingsAutoDebetTemp($auth['branch_id']);

			$data['main_view']['content']							= 'AcctSavingsAutoDebet/ListDataAcctSavingsAutoDebet_view';
			$this->load->view('MainPage_view',$data);
		}

		public function processUpdateAcctSavingsAutoDebet(){
			$auth = $this->session->userdata('auth');

			// $auth['branch_id'] = 2;

			$periode 	= $this->AcctSavingsAutoDebet_model->getPeriode($auth);
			...
			$data_log_step5 = $this->AcctSavingsAutoDebet_model->getDataLogStep5($log_step5);
			...
			if(empty($data_log_step5)){	
				if($this->AcctSavingsAutoDebet_model->insertDataLogStep($log_step5)){

					$this->AcctSavingsAutoDebet_model->insertSystemPeriodLog($dataperiod);
					if($this->AcctSavingsAutoDebet_model->insertAcctSavingsAutoDebetFix()){

						$corebranch = $this->AcctSavingsAutoDebet_model->getCoreBranch();

						foreach ($corebranch as $key => $vCB) {
							$savings_profit_sharing_amount = $this->AcctSavingsAutoDebet_model->getTotalSavingsProfitSharing($vCB['branch_id']);
							$data_transfer = array (
							...
							if($this->AcctSavingsAutoDebet_model->insertAcctSavingsTransferMutation($data_transfer)){
								$savings_transfer_mutation_id = $this->AcctSavingsAutoDebet_model->getSavingsTranferMutationID($data_transfer['created_id']);

								$this->AcctSavingsAutoDebet_model->insertAcctSavingsTransferMutationTo($savings_transfer_mutation_id, $vCB['branch_id']);
							}

							$acctsavings 	= $this->AcctSavingsAutoDebet_model->getAcctSavings();


							//-------------------------------------Jurnal--------------------------------------------------------//
							foreach ($acctsavings as $key => $val) {
								$totalsavingsprofitsharing 				= $this->AcctSavingsAutoDebet_model->getSubTotalSavingsProfitSharing($val['savings_id'], $vCB['branch_id']);

								$totalsavingsprofitsharingallocation 	= $this->AcctSavingsAutoDebet_model->getSubTotalSavingsProfitSharingAllocation($val['savings_id'], $vCB['branch_id']);

								$preferenceprofitsharingallocation 		= $this->AcctSavingsAutoDebet_model->getPreferenceProfitSharingAllocation();
								$preferenceprofitsharing 				= $this->AcctSavingsAutoDebet_model->getPreferenceProfitSharing();
								...
								$journal_voucher_id 			= $this->AcctSavingsAutoDebet_model->getJournalVoucherID($data_journal['created_id']);

								$account_basil_id 				= $this->AcctSavingsAutoDebet_model->getAccountBasilID($val['savings_id']);

								$account_id_default_status 		= $this->AcctSavingsAutoDebet_model->getAccountIDDefaultStatus($account_basil_id);
								...
								$account_id 					= $this->AcctSavingsAutoDebet_model->getAccountID($val['savings_id']);
								...
								if(!empty($preferenceprofitsharingallocation)){
									foreach ($preferenceprofitsharingallocation as $kA => $vA){
										...
										$account_id_default_status 		= $this->AcctSavingsAutoDebet_model->getAccountIDDefaultStatus($vA['account_id']);
		...
		public function cekSRH(){
			...
			$acctsavingsaccountdetail 	= $this->AcctSavingsAutoDebet_model->getAcctSavingsAccountDetail($data['month_period'], $savings_account_id);
			...
			$data['main_view']['content']			= 'AcctSavingsAutoDebet/ListTestSRH_view';
			$this->load->view('MainPage_view',$data);
		}
	}	
?>