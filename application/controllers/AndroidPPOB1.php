<?php
	Class AndroidPPOB extends CI_Controller{
		...
		// SALDO PPOB
			...
			public function getPPOBBalance(){
				...
				if($response["error"] == FALSE){
					...
					$ppob_balance 		= $this->AndroidPPOB_model->getPPOBBalance($ppob_company_id, $ppob_agen_id);
		...
		// TOPUP PPOB 
		public function processPpobTopupMember(){
			$data = array(
				...
				'ppob_topup_member_amount'	=> $this->input->post('savings_transfer_mutation_amount', true),
			...
			/* $data = array(
				...
				'branch_id'					=> 2,
				'ppob_topup_member_date'	=> date('Y-m-d'),
				'ppob_topup_member_amount'	=> 25000,
				'created_id'				=> 32887,
				'created_on'				=> date('Y-m-d H:i:s'),
			); */

			/* savings_from_id=14&savings_account_from_id=31014&savings_transfer_mutation_amount=25000&savings_account_from_opening_balance=137103.56&username=00000001&member_from_id=32887&user_id=32887&branch_from_id=2 */
			...
			//saldo ppob madani
			$ppob_company_balance	= $this->AndroidPPOB_model->getPPOBCompanyBalance($ppob_company_id);
			...
			print_r("ppob_company_id ");
			print_r($ppob_company_id);
			print_r("<BR> ");
			print_r("<BR> ");

			print_r("ppob_company_balance ");
			print_r($ppob_company_balance);
			print_r("<BR> ");
			print_r("<BR> ");
			exit; */
			...
			if($response["error_insertacctsavingsppob"] == FALSE){
				if(!empty($data)){
					if($data['ppob_topup_member_amount'] > $ppob_company_balance){
						...
						if($this->PpobTopupMember_model->insertPpobTopupMember($data)){
							...
							$ppobtopupmember_last 			= $this->PpobTopupMember_model->getPpobTopupMember_Last($data['created_id']);

							$coremember 					= $this->PpobTopupMember_model->getCoreMember_Detail($data['member_id']);

							$journal_voucher_period			= '202004';

							$data_journal = array(
								'branch_id'							=> $data['branch_id'],
								'journal_voucher_period' 			=> $journal_voucher_period,
								'journal_voucher_date'				=> date('Y-m-d'),
								'journal_voucher_title'				=> 'TOP UP PPOB '.$coremember['member_name'],
								'journal_voucher_description'		=> 'TOP UP PPOB '.$coremember['member_name'],
							...
							$data_debet = array (
								...
								'journal_voucher_description'	=> 'TOP UP PPOB '.$coremember['member_name'],
								'journal_voucher_amount'		=> $data['ppob_topup_member_amount'],
								'journal_voucher_debit_amount'	=> $data['ppob_topup_member_amount'],
							...
							$data_credit =array(
								...
								'journal_voucher_description'	=> 'TOP UP PPOB '.$coremember['member_name'],
								'journal_voucher_amount'		=> $data['ppob_topup_member_amount'],
								'journal_voucher_credit_amount'	=> $data['ppob_topup_member_amount'],
								'account_id_default_status'		=> $account_id_default_status,
								'account_id_status'				=> 1,
							);
		...
		// 	if($response["error_insertacctsavingsppob"] == FALSE){
		// 		if(!empty($data)){	
		// 			$check_topupppob = $this->AndroidPPOB_model->checkAcctSavingsTransfer($savings_account_from_id, $preferencecompany['savings_account_ppob_id']);
		...
		// 					$this->AndroidPPOB_model->insertPPOBTopUP($data_ppob);
		...
		public function getPPOBTransaction(){
			$base_url 	= base_url();
			$auth 		= $this->session->userdata('auth');
			...
			if($response["error"] == FALSE){
				...
				/* $ppobtransactionlog = $this->AndroidPPOB_model->getPPOBTransactionLog($ppob_company_id, $data['user_id']); */

				$ppobtransaction 	= $this->AndroidPPOB_model->getPPOBTransaction($ppob_company_id, $data['member_id'], $data['ppob_status']);

				/* print_r("ppobtransaction");
				print_r($ppobtransaction);
				print_r("<BR> ");
				print_r("<BR> "); */
				...
				if(!$ppobtransaction){
					...
					if (empty($ppobtransaction)){
						...
						foreach ($ppobtransaction as $key => $val) {
							...
							$ppobtransaction[$key]['ppob_transaction_status_name']		= $ppobstatus[$val['ppob_transaction_status']];
		...
		public function getPPOBPulsaPascabayar(){
			...
			$ppobproductlist 			= $this->AndroidPPOB_model->getPPOBProduct($ppob_product_category_id);
		...
		public function getPPOBPLNPrepaidData(){
			...
			$ppobtransaction_plnprepaid	= $this->AndroidPPOB_model->getPPOBTransaction_Product($ppob_company_id, $ppob_agen_id, $ppobproduct['ppob_product_id']);
		...
		public function getPPOBBalanceSavingsAccount(){
			$base_url 	= base_url();
			$auth 		= $this->session->userdata('auth');
			...
			if($response["error"] == FALSE){
				$ppobbalancesavingsaccountlist	= $this->AndroidPPOB_model->getPPOBBalanceSavingsAccount($data['member_id']);
				
				if(!$ppobbalancesavingsaccountlist){
					$response['error'] 				= TRUE;
					$response['error_msg_title'] 	= "No Data";
					$response['error_msg'] 			= "Error Query Data";
				}else{
					if (empty($ppobbalancesavingsaccountlist)){
						$response['error'] 				= TRUE;
						$response['error_msg_title'] 	= "No Data";
						$response['error_msg'] 			= "Data Does Not Exist";
					} else {
						if (!empty($ppobbalancesavingsaccountlist)){
							$ppobbalancesavingsaccount[0]['member_id']						= $ppobbalancesavingsaccountlist['member_id'];
							$ppobbalancesavingsaccount[0]['savings_account_id']				= $ppobbalancesavingsaccountlist['savings_account_id'];
							$ppobbalancesavingsaccount[0]['savings_account_no']				= $ppobbalancesavingsaccountlist['savings_account_no'];
							$ppobbalancesavingsaccount[0]['savings_id']						= $ppobbalancesavingsaccountlist['savings_id'];
							$ppobbalancesavingsaccount[0]['savings_name']					= $ppobbalancesavingsaccountlist['savings_name'];
							$ppobbalancesavingsaccount[0]['savings_account_last_balance']	= $ppobbalancesavingsaccountlist['savings_account_last_balance'];
						}
							
						$response['error'] 								= FALSE;
						$response['error_msg_title'] 					= "Success";
						$response['error_msg'] 							= "Data Exist";
						$response['ppobbalancesavingsaccount'] 			= $ppobbalancesavingsaccount;
					}
				}
			}
		...
		// HISTORI TRANSAKSI PPOB MASUK

		public function getAcctSavingsAccountPPOBInHistory(){
			$base_url 	= base_url();
			$auth 		= $this->session->userdata('auth');
			...
			$response = array(
				...
				'error_msg'							=> "",
				'error_msg_title'					=> "",
				'acctsavingsaccountppobinhistory'	=> "",
			);
			...
			if($response["error"] == FALSE){
				...
				$preferencecompany 				= $this->AndroidPPOB_model->getPreferenceCompany();

				$acctsavingsaccountlist			= $this->AndroidPPOB_model->getAcctSavingsTransferMutationTo_History($data['member_id']);
				...
				if(!$acctsavingsaccountlist){
					...
					if (empty($acctsavingsaccountlist)){
						...
						foreach ($acctsavingsaccountlist as $key => $val){
							$acctsavingsaccountppobinhistory[$key]['ppob_transaction_title']		= 'PPOB Masuk';
							$acctsavingsaccountppobinhistory[$key]['ppob_transaction_date']			= date('d M Y H:i:s', strtotime($val['created_on']));
							$acctsavingsaccountppobinhistory[$key]['ppob_transaction_description']	= 'Bagi Hasil PPOB Ke Rekening '.$val['savings_account_no'].' a/n '.$val['member_name'];
							$acctsavingsaccountppobinhistory[$key]['ppob_transaction_amount']		= $val['savings_transfer_mutation_to_amount'];
						}
							
						$response['error'] 								= FALSE;
						$response['error_msg_title'] 					= "Success";
						$response['error_msg'] 							= "Data Exist";
						$response['acctsavingsaccountppobinhistory'] 	= $acctsavingsaccountppobinhistory;
					}
				}
			}
		...
		// END HISTORI TRANSAKSI PPOB MASUK



		// HISTORI TRANSAKSI PPOB KELUAR

			public function getAcctSavingsAccountPPOBOutHistory(){
				$base_url 	= base_url();
				$auth 		= $this->session->userdata('auth');
				...
				$response = array(
					...
					'error_msg'								=> "",
					'error_msg_title'						=> "",
					'acctsavingsaccountppobouthistory'		=> "",
				);
				...
				if($response["error"] == FALSE){
					...
					$acctsavingsaccountlist			= $this->AndroidPPOB_model->getAcctSavingsTransferMutationFrom_History($data['member_id']);
					...
					if(!$acctsavingsaccountlist){
						...
						if (empty($acctsavingsaccountlist)){
							...
							foreach ($acctsavingsaccountlist as $key => $val){
								$acctsavingsaccountppobouthistory[$key]['ppob_transaction_title']			= 'PPOB Keluar';
								$acctsavingsaccountppobouthistory[$key]['ppob_transaction_date']			= date('d M Y H:i:s', strtotime($val['created_on']));
								$acctsavingsaccountppobouthistory[$key]['ppob_transaction_description']		= 'Transaksi PPOB Dari Rekening '.$val['savings_account_no'].' a/n '.$val['member_name'];
								$acctsavingsaccountppobouthistory[$key]['ppob_transaction_amount']			= $val['savings_transfer_mutation_from_amount'];
							}
								
							$response['error'] 									= FALSE;
							$response['error_msg_title'] 						= "Success";
							$response['error_msg'] 								= "Data Exist";
							$response['acctsavingsaccountppobouthistory'] 	= $acctsavingsaccountppobouthistory;
						}
					}
				}
		...
		// END HISTORI TRANSAKSI PPOB KELUAR
