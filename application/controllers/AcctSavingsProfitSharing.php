	...
	Class AcctSavingsProfitSharing extends CI_Controller{
		...
		public function processAddAcctSavingsProfitSharing(){
			...
			if($this->form_validation->run()==true){
					...
					$acctsavingsprofitsharing 	= $this->AcctSavingsProfitSharing_model->getAcctSavingsProfitSharingLog_Detail($data_log['branch_id'], $data_log['savings_id'], $savings_profit_sharing_period);
						...
						if($this->AcctSavingsProfitSharing_model->insertAcctSavingsProfitSharingLog($data_log)){
							...
							foreach ($acctsavingsaccount as $k => $v) {
								// if($v['savings_account_daily_average_balance'] >= $data['savings_daily_average_balance_minimum']){
									$savings_index_amount 			= $this->AcctSavingsProfitSharing_model->getSavingsIndexAmount($data['savings_id'], $savings_profit_sharing_period);

									$savings_profit_sharing_amount 	= $v['savings_account_daily_average_balance'] * $savings_index_amount;
