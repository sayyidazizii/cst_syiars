<?php
	Class AcctSavingsProfitSharingReport extends CI_Controller{
		...
		public function index(){
			...
			$sesi = $this->session->userdata('filter-acctsavingsprofitsharingreport');
			if(!is_array($sesi)){
			...
			$list 		= $this->AcctSavingsProfitSharingReport_model->get_datatables($period, $branch_id);
			...
			$corebranch 									= create_double_branch($this->AcctSavingsProfitSharingReport_model->getCoreBranch(),'branch_id','branch_name');
		...
		public function filter(){
			...
			$this->session->set_userdata('filter-acctsavingsprofitsharingreport',$data);
			redirect('AcctSavingsProfitSharingReport');
		}

		public function getSavingsProfitSharing(){
			$auth = $this->session->userdata('auth');
			$sesi = $this->session->userdata('filter-acctsavingsprofitsharingreport');
			if(!is_array($sesi)){
				$sesi['month_period']		= date('m');
	        ...
	        foreach ($list as $profitsharing) {
	        	if($profitsharing->savings_profit_sharing_amount >= 240000){
	        		$pajak = ($profitsharing->savings_profit_sharing_amount * 10) / 100;
	        	} else {
	        		$pajak = 0;
	        	}
	            ...
	            $row[] = number_format($profitsharing->savings_profit_sharing_amount, 2);
	        ...
	        $output = array(
	                        "draw" => $_POST['draw'],
	                        "recordsTotal" => $this->AcctSavingsProfitSharingReport_model->count_all($period, $branch_id),
	                        "recordsFiltered" => $this->AcctSavingsProfitSharingReport_model->count_filtered($period, $branch_id),
	                        "data" => $data,
	                );
		...
		public function view(){
			...
			foreach ($list as $profitsharing) {
	            ...
	            $data[] = array(
	            	...
	            	'savings_profit_sharing_amount'		=> $profitsharing->savings_profit_sharing_amount,
	            	'tax'								=> $pajak,
	            );
			}
		...
		public function processPrinting($data){
			...
			<table cellspacing=\"0\" cellpadding=\"1\" border=\"0\" width=\"100%\">
			    <tr>
			        ...
			        <td width=\"15%\"><div style=\"text-align: center;border-bottom: 1px solid black;border-top: 1px solid black\">Basil</div></td>
			        <td width=\"15%\"><div style=\"text-align: center;border-bottom: 1px solid black;border-top: 1px solid black\">Pajak</div></td>
			    </tr>			
			</table>";
			...
			foreach ($data as $key => $val) {
				$tbl3 .= "
					<tr>
				        ...
				        <td width=\"15%\"><div style=\"text-align: right;\">".number_format($val['tax'], 2)."</div></td>
				    </tr>
				";
			...
			$tbl4 = "
					<tr>
						...
						<td style=\"border-top: 1px solid black\"><div style=\"font-size:10;text-align:right\">".number_format($total_pajak, 2)."</div></td>
			...
			$filename = 'Laporan Basil Periode '.$period.'.pdf';
