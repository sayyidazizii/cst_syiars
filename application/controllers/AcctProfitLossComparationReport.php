...
<?php
	Class AcctProfitLossComparationReport extends CI_Controller{
		...
		public function index(){
			...
			$data['main_view']['acctprofitlosscomparationreport_top']		= $this->AcctProfitLossComparationReport_model->getAcctProfitLossComparationReport_Top();
		...
		public function filter(){
			...
			redirect('AcctProfitLossComparationReport');
		}
		...
		public function processPrinting(){
			...
			$acctprofitlosscomparationreport_top		= $this->AcctProfitLossComparationReport_model->getAcctProfitLossComparationReport_Top();
			...
			$tblFooter = "
			    ...
			    <tr>
			    	<td width=\"50%\" style=\"border:1px black solid;\">
			    		<table id=\"items\" width=\"100%\" cellspacing=\"1\" cellpadding=\"2\" border=\"0\">
			    			<tr>
								<td style=\"width: 60%\"><div style=\"font-weight:bold;font-size:14px\">SHU</div></td>
		...
		public function exportAcctProfitLossComparationReport(){
			...
			if(!empty($acctprofitlosscomparationreport_top && $acctprofitlosscomparationreport_bottom)){
						...
						if($valBottom['report_type'] == 6){
							if(!empty($valBottom['report_formula']) && !empty($valBottom['report_operator'])){
								...
								for($i = 0; $i < count($report_formula); $i++){
									if($report_operator[$i] == '-'){
										if($value == 0 ){
											$grand_total_account_amount2_bottom_before = $grand_total_account_amount2_bottom_before + $account_amount[$report_formula[$i]];
										} else {
											$grand_total_account_amount2_bottom_before = $grand_total_account_amount2_bottom_before - $account_amount[$report_formula[$i]];
										}
									} else if($report_operator[$i] == '+'){
				...
				$this->excel->getActiveSheet()->setCellValue('B'.$total_row, "SHU");
				$this->excel->getActiveSheet()->setCellValue('C'.$total_row, $shu_before);

				$this->excel->getActiveSheet()->setCellValue('D'.$total_row, "SHU");
				$this->excel->getActiveSheet()->setCellValue('E'.$total_row, $shu_now);
