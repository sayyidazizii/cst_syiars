<?php
	Class AcctCredits extends CI_Controller{
		...
		public function reset_data(){
			...
			redirect('AcctCredits/addAcctCredits');
		}
		...
		public function processAddAcctCredits(){
			...
			$this->form_validation->set_rules('credits_code', 'Kode Pembiayaan', 'required');
			$this->form_validation->set_rules('credits_name', 'Nama Pembiayaan', 'required');
			...
			if($this->form_validation->run()==true){
				if($this->AcctCredits_model->insertAcctCredits($data)){
					...
					$msg = "<div class='alert alert-success alert-dismissable'>  
							<button type='button' class='close' data-dismiss='alert' aria-hidden='true'></button>					
								Tambah Data Kode Pembiayaan Sukses
							</div> ";
					...
					$msg = "<div class='alert alert-danger alert-dismissable'>
							<button type='button' class='close' data-dismiss='alert' aria-hidden='true'></button>					
								Tambah Data Kode Pembiayaan Tidak Berhasil
							</div> ";
					$this->session->set_userdata('message',$msg);
					redirect('AcctCredits/addAcctCredits');
				}
		...
		public function processEditAcctCredits(){
			...
			if($this->form_validation->run()==true){
				if($this->AcctCredits_model->updateAcctCredits($data)){
					...
					$msg = "<div class='alert alert-success alert-dismissable'>  
							<button type='button' class='close' data-dismiss='alert' aria-hidden='true'></button>
								Edit Kode Pembiayaan Sukses
							</div> ";

					$this->session->set_userdata('message',$msg);
					redirect('AcctCredits/editAcctCredits/'.$data['credits_id']);
				}else{
					$msg = "<div class='alert alert-danger alert-dismissable'> 
							<button type='button' class='close' data-dismiss='alert' aria-hidden='true'></button>
								Edit Kode Pembiayaan Tidak Berhasil
							</div> ";
					$this->session->set_userdata('message',$msg);
					redirect('AcctCredits/editAcctCredits/'.$data['credits_id']);
				}
		...
		public function deleteAcctCredits(){
			if($this->AcctCredits_model->deleteAcctCredits($this->uri->segment(3))){
				...
				$msg = "<div class='alert alert-success alert-dismissable'>                 
							Hapus Data Kode Pembiayaan Sukses
						</div> ";
				$this->session->set_userdata('message',$msg);
				redirect('AcctCredits');
			}else{
				$msg = "<div class='alert alert-danger alert-dismissable'>                
							Hapus Data Kode Pembiayaan Tidak Berhasil
						</div> ";
				$this->session->set_userdata('message',$msg);
				redirect('AcctCredits');
			}
