<?php
	Class AcctCashPaymentsBranch extends CI_Controller{
		...
		public function filteracctcashpayment(){
			...
			redirect('AcctCashPaymentsBranch/indAcctCashPayment');
		}
		...
		public function getAcctCashPayment(){
	        ...
	        foreach ($list as $cashpayment) {
			    ...
			    $row[] = '<a href="'.base_url().'AcctCashPaymentsBranch/printNoteCashPayment/'.$cashpayment->credits_payment_id.'" class="btn btn-xs blue" role="button"><i class="fa fa-print"></i> Kwitansi</a>';
	            $data[] = $row;
	        }
		...
		public function akadlisttunai(){
	        ...
	        foreach ($list as $customers) {
	            ...
	            $row[] = tgltoview($customers->credits_account_due_date);
	             $row[] = '<a href="'.base_url().'AcctCashPaymentsBranch/addAcctCashPayment/'.$customers->credits_account_id.'" class="btn btn-info" role="button"><span class="glyphicon glyphicon-ok"></span> Select</a>';
		...
		public function processAddAcctCashPayment(){
			...
			// if($data['credits_payment_amount'] != ''){
				if($this->AcctCashPaymentBranch_model->insert($data)){
					...
					redirect('AcctCashPaymentsBranch/printNoteCashPayment/'.$acctcashpayment_last['credits_payment_id']);
				}else{
