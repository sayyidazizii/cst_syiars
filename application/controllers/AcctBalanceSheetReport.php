<?php ob_start();?>
<?php
	Class AcctBalanceSheetReport extends CI_Controller{
		public function __construct(){
			parent::__construct();
			$this->load->model('Connection_model');
			$this->load->model('MainPage_model');
			$this->load->model('AcctBalanceSheetReport_model');
		...
		public function index(){
			...
			if($auth['branch_status'] == 1){
				$sesi	= 	$this->session->userdata('filter-AcctBalanceSheetReport');
				// if(!is_array($sesi)){
			...
			$data['main_view']['corebranch']					= create_double($this->AcctBalanceSheetReport_model->getCoreBranch(),'branch_id','branch_name');
			$data['main_view']['acctbalancesheetreport_left']	= $this->AcctBalanceSheetReport_model->getAcctBalanceSheetReport_Left();

			$data['main_view']['acctbalancesheetreport_right']	= $this->AcctBalanceSheetReport_model->getAcctBalanceSheetReport_Right();
			$data['main_view']['content']						= 'AcctBalanceSheetReport/AcctBalanceSheetReport_view';
		...
		public function filter(){
			...
			$this->session->set_userdata('filter-AcctBalanceSheetReport',$data);
			redirect('AcctBalanceSheetReport');
		}
		...
		public function processPrinting(){
			...
			$acctbalancesheetreport_left	= $this->AcctBalanceSheetReport_model->getAcctBalanceSheetReport_Left();

			/*print_r("acctbalancesheetreport_left ");
			print_r($acctbalancesheetreport_left);
			exit;*/

			$acctbalancesheetreport_right	= $this->AcctBalanceSheetReport_model->getAcctBalanceSheetReport_Right();
			...
			$day 	= date("d", strtotime($sesi['start_date']));
			...
			$tbl = "
				<table cellspacing=\"0\" cellpadding=\"5\" border=\"0\">
				    <tr>
				        <td colspan=\"5\"><div style=\"text-align: center; font-size:14px\">LAPORAN NERACA <BR>".$preferencecompany['company_name']." <BR>Periode ".$period."</div></td>
				    </tr>
				</table>
			";
			        		...
			        		<table id=\"items\" width=\"100%\" cellspacing=\"1\" cellpadding=\"2\" border=\"0\">";	
			        			...
			        			foreach ($acctbalancesheetreport_left as $keyLeft => $valLeft) {
									...
									if($valLeft['report_type1']	== 2){
										$tblitem_left2 = "
											<tr>
												<td style=\"width: 68%\"><div style='font-weight:".$report_bold1."'>".$report_tab1."".$valLeft['account_name1']."</div></td>
									...
									if($valLeft['report_type1']	== 3){
										$last_balance1 = $this->AcctBalanceSheetReport_model->getLastBalance($valLeft['account_id1'], $sesi['branch_id'], $sesi['start_date']);		
									...
									if($valLeft['report_type1']	== 7){
										$last_balance7 = $this->AcctBalanceSheetReport_model->getLastBalance($valLeft['account_id1'], $sesi['branch_id'], $sesi['start_date']);		

										$tblitem_left7 = "
											<tr>
												<td><div style='font-weight:".$report_bold1."'>".$report_tab1."(".$valLeft['account_code1'].") ".$valLeft['account_name1']."</div> </td>
												<td style=\"text-align:right;\">(".number_format($last_balance7, 2).")</td>
											</tr>";

										$account_amount1_top[$valLeft['report_no']] = $last_balance7;

									} else {
										$tblitem_left7 = "";
									}

									/*print_r("tblitem_left7 ");
									print_r($tblitem_left7);
									exit;*/
			        		...
			        		<table id=\"items\" width=\"100%\" cellspacing=\"1\" cellpadding=\"2\" border=\"0\">";		
			        			...
			        			foreach ($acctbalancesheetreport_right as $keyRight => $valRight) {
									...
									if($valRight['report_type2'] == 2){
										$tblitem_right2 = "
											<tr>
												<td style=\"width: 68%\"><div style='font-weight:".$report_bold2."'>".$report_tab2."".$valRight['account_name2']."</div></td>
									...
									if($valRight['report_type2']	== 3){
										$last_balance2 = $this->AcctBalanceSheetReport_model->getLastBalance($valRight['account_id2'], $sesi['branch_id'], $sesi['start_date']);
			    	...
			    	<td style=\"width: 50%\">
			    		<table id=\"items\" width=\"100%\" cellspacing=\"1\" cellpadding=\"1\" border=\"0\">
			    			<tr>
								...
								<td style=\"width: 45%; text-align:right;\"><div style=\"font-weight:".$report_bold1."; font-size:14px\">".number_format($total_account_amount1, 2)."</div></td>
							</tr>
			    		</table>
			    	</td>
			    	<td style=\"width: 50%\">
			    		<table id=\"items\" width=\"100%\" cellspacing=\"1\" cellpadding=\"1\" border=\"0\">
			    			<tr>
								...
								<td style=\"width: 45%; text-align:right;\"><div style=\"font-weight:".$report_bold2."; font-size:14px\">".number_format($total_account_amount2, 2)."</div></td>
							</tr>
			    		</table>
			    	</td>
			    </tr>
				...
				foreach($acctbalancesheetreport_left as $keyLeft =>$valLeft){
					if(is_numeric($keyLeft)){
						...
						if($valLeft['report_type1']	== 7){
							...
							if (empty($last_balance7)){
								$last_balance1 = 0;
							}

							$this->excel->getActiveSheet()->setCellValue('B'.$j, $report_tab1.$valLeft['account_name1']);
							$this->excel->getActiveSheet()->setCellValue('C'.$j, $report_tab1."(".$last_balance7.")");
