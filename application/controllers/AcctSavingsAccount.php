<?php
	...
	Class AcctSavingsAccount extends CI_Controller{
		...
		public function getAcctSavingsAccountList(){
			...
			foreach ($list as $savingsaccount) {
				...
				if($savingsaccount->validation == 0){
					$row[] = '<a href="'.base_url().'AcctSavingsAccount/printNoteAcctSavingsAccount/'.$savingsaccount->savings_account_id.'" class="btn btn-xs blue" role="button"><i class="fa fa-print"></i> Kwitansi</a>
						<a href="'.base_url().'AcctSavingsAccount/validationAcctSavingsAccount/'.$savingsaccount->savings_account_id.'" class="btn btn-xs green-jungle" role="button"><i class="fa fa-check"></i> Validasi</a>
						<a href="'.base_url().'AcctSavingsAccount/editAcctSavingsAccount/'.$savingsaccount->savings_account_id.'" class="btn btn-xs purple" role="button"><i class="fa fa-edit"></i> Edit</a>';
						// <a href="'.base_url().'AcctSavingsAccount/voidAcctSavingsAccount/'.$savingsaccount->savings_account_id.'" class="btn btn-xs red" role="button"><i class="fa fa-trash-o"></i> Batal</a>';
				} else {
					$row[] = '<a href="'.base_url().'AcctSavingsAccount/printNoteAcctSavingsAccount/'.$savingsaccount->savings_account_id.'" class="btn btn-xs blue" role="button"><i class="fa fa-print"></i> Kwitansi</a>
						<a href="'.base_url().'AcctSavingsAccount/editAcctSavingsAccount/'.$savingsaccount->savings_account_id.'" class="btn btn-xs purple" role="button"><i class="fa fa-edit"></i> Edit</a>';
						// <a href="'.base_url().'AcctSavingsAccount/voidAcctSavingsAccount/'.$savingsaccount->savings_account_id.'" class="btn btn-xs red" role="button"><i class="fa fa-trash-o"></i> Batal</a>';
		...
		public function filtermasterdata(){
			...
			redirect('AcctSavingsAccount/getMasterDataSavingsAccount');
		}
		...
		public function block(){
			...
			$data = array(
				...
				'blocked'					=> 1,
				'blocked_id'				=> $auth['user_id'],
				'blocked_on'				=> date('Y-m-d H:i:s'),
			);

			if($this->AcctSavingsAccount_model->updateAcctSavingsAccount($data)){
				$msg = "<div class='alert alert-success alert-dismissable'>  
				<button type='button' class='close' data-dismiss='alert' aria-hidden='true'></button>					
					Blokir Data Rekening Simpanan Sukses
				</div> ";
				...
				<button type='button' class='close' data-dismiss='alert' aria-hidden='true'></button>					
					Blokir Data Rekening Simpanan Gagal
				</div> ";
				$this->session->set_userdata('message',$msg);
				redirect('AcctSavingsAccount/getMasterDataSavingsAccount');
			}
		...
		public function openBlock(){
			...
			if($this->AcctSavingsAccount_model->updateAcctSavingsAccount($data)){
				$msg = "<div class='alert alert-success alert-dismissable'>  
				<button type='button' class='close' data-dismiss='alert' aria-hidden='true'></button>					
					Buka Blokir Data Rekening Simpanan Sukses
				</div> ";
				$this->session->set_userdata('message',$msg);
				redirect('AcctSavingsAccount/getMasterDataSavingsAccount');
			}else{
				$msg = "<div class='alert alert-danger alert-dismissable'>  
				<button type='button' class='close' data-dismiss='alert' aria-hidden='true'></button>					
					Buka Blokir Data Rekening Simpanan Gagal
				</div> ";
				$this->session->set_userdata('message',$msg);
				redirect('AcctSavingsAccount/getMasterDataSavingsAccount');
			}
		...
		public function getMasterDataSavingsAccountList(){
			...
			foreach ($list as $savingsaccount) {
				...
				if($savingsaccount->blocked==0){
					$row[] = '<a href="'.base_url().'AcctSavingsAccount/block/'.$savingsaccount->savings_account_id.'" class="btn default btn-xs red", onClick="javascript:return confirm(\'apakah yakin ingin diblokir ?\')" role="button"><i class="fa fa-times"></i> Blokir</a>';
				}else{
					$row[] = '<a href="'.base_url().'AcctSavingsAccount/openBlock/'.$savingsaccount->savings_account_id.'" class="btn default btn-xs yellow", onClick="javascript:return confirm(\'apakah yakin ingin buka blokir ?\')" role="button"><i class="fa fa-check"></i> Buka Blokir</a>';
				}
		...
		public function export(){
			...
			foreach ($list as $savingsaccount) {
				...
				$data[] = array(
					...
					'member_no' 							=> $savingsaccount->member_no,	
			...
			$this->exportMasterDataAcctSavingsAccount($datacetak);
		}
		...
		public function reset_data(){
			...
			redirect('AcctSavingsAccount/addAcctSavingsAccount');
		}
		...
		public function getListCoreMember(){
			...
			foreach ($list as $customers) {
				...
				$row[] = '<a href="'.base_url().'AcctSavingsAccount/addAcctSavingsAccount/'.$customers->member_id.'" class="btn btn-info" role="button"><span class="glyphicon glyphicon-ok"></span> Select</a>';
				$data[] = $row;
			}
		...
		public function getSavingsAccountNo(){
			...
			$savingsnisbah 			= $this->AcctSavingsAccount_model->getSavingsNisbah($savings_id);
			...
			if($this->form_validation->run()==true){
				if($savings_account_token->num_rows() == 0){
					if($this->AcctSavingsAccount_model->insertAcctSavingsAccount($data)){
						...
						$this->session->unset_userdata('acctsavingsaccounttoken-'.$sesi['unique']);
						$this->session->set_userdata('message',$msg);
						redirect('AcctSavingsAccount/printNoteAcctSavingsAccount/'.$acctsavingsaccount_last['savings_account_id']);
					}else{
		...
		public function validationAcctSavingsAccount(){
			...
			if($this->AcctSavingsAccount_model->validationAcctSavingsAccount($data)){
				...
				$this->session->set_userdata('message',$msg);
				redirect('AcctSavingsAccount/printValidationAcctSavingsAccount/'.$savings_account_id);
			}else{
				$msg = "<div class='alert alert-danger alert-dismissable'> 
						<button type='button' class='close' data-dismiss='alert' aria-hidden='true'></button>
		...
		public function exportMasterDataAcctSavingsAccount($data){	
			$auth 	= $this->session->userdata('auth');
			$sesi	= $this->session->userdata('filter-masterdataacctsavingsaccount');
			if(!is_array($sesi)){
				$sesi['start_date']		= date('Y-m-d');
			...
			if(count($data) !=0){
				...
				$this->excel->getActiveSheet()->setCellValue('E3',"Alamat Anggota");
				$this->excel->getActiveSheet()->setCellValue('F3',"Jenis Simpanan");
				$this->excel->getActiveSheet()->setCellValue('G3',"No. Rekening");
				$this->excel->getActiveSheet()->setCellValue('H3',"Tanggal Buka");
				$this->excel->getActiveSheet()->setCellValue('I3',"Setoran Awal");
				$this->excel->getActiveSheet()->setCellValue('J3',"Saldo");
				
				$j=4;
				$no=0;
				
				foreach($data as $key=>$val){
					$city_name = $this->AcctSavingsAccount_model->getCityName($val['city_id']);
					$kecamatan_name = $this->AcctSavingsAccount_model->getKecamatanName($val['kecamatan_id']);
					$kelurahan_name = $this->AcctSavingsAccount_model->getKelurahanName($val['kelurahan_id']);
					...
					if(is_numeric($key)){
						...
						$this->excel->getActiveSheet()->setCellValue('E'.$j, $val['member_address'].', '.$kelurahan_name.', '.$kecamatan_name.', '.$city_name);
						$this->excel->getActiveSheet()->setCellValue('F'.$j, $val['savings_name']);
						$this->excel->getActiveSheet()->setCellValue('G'.$j, $val['savings_account_no']);
						$this->excel->getActiveSheet()->setCellValue('H'.$j, tgltoview($val['savings_account_date']));
						$this->excel->getActiveSheet()->setCellValue('I'.$j, $val['savings_account_first_deposit_amount']);
		...
		public function editAcctSavingsAccount(){
			$data['main_view']['membergender']				= $this->configuration->MemberGender();
			$data['main_view']['memberidentity']			= $this->configuration->MemberIdentity();
			$data['main_view']['acctsavingsaccount']		= $this->AcctSavingsAccount_model->getAcctSavingsAccount_Detail($this->uri->segment(3));
			$data['main_view']['content']					= 'AcctSavingsAccount/FormEditAcctSavingsAccount_view';
			$this->load->view('MainPage_view',$data);
		}

		public function processEditAcctSavingsAccount(){
			$auth	= $this->session->userdata('auth');
			...
			if($this->form_validation->run()==true){
				if($this->AcctSavingsAccount_model->updateAcctSavingsAccount($newdata)){
					$msg = "<div class='alert alert-success alert-dismissable'>  
							<button type='button' class='close' data-dismiss='alert' aria-hidden='true'></button>
								Perubahan No. Rekening Sukses
							</div>";
					$this->session->set_userdata('message',$msg);
					redirect('AcctSavingsAccount/editAcctSavingsAccount/'.$newdata['savings_account_id']);
				}else{
					$msg = "<div class='alert alert-danger alert-dismissable'> 
							<button type='button' class='close' data-dismiss='alert' aria-hidden='true'></button>
