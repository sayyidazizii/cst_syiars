	...
	Class CoreOffice extends CI_Controller{
		...
		public function addCoreDusun(){
			...
			if(empty($data['kecamatan_id']) || $data['kecamatan_id'] == ''){
				...
				$datadusun = array(
					'record_id'				=> $this->input->post('dusun_id', true).$date,
					'kelurahan_id'			=> $this->input->post('kelurahan_id', true),
					'dusun_id'				=> $this->input->post('dusun_id', true),
				);
			}
			...
			if($this->form_validation->run()==true){
				...
				if(!empty($data['dusun_id']) || $data['dusun_id'] != ''){
					$dataArrayHeader[$datadusun['record_id']] = $datadusun;
				} else {
					$dataArrayHeader = $datadusun;
				}
		...
		public function deleteCoreDusun(){
			...
			redirect('CoreOffice/addCoreOffice/');
		}
		...
		public function editCoreDusun(){
			...
			if(empty($data['kecamatan_id']) || $data['kecamatan_id'] == ''){
				...
				$datadusun = array(
					'user_dusun_id'			=> $this->input->post('dusun_id', true).$date,
					'user_id'				=> $this->input->post('user_id', true),
					'kelurahan_id'			=> $this->input->post('kelurahan_id', true),
					'dusun_id'				=> $this->input->post('dusun_id', true),
				);
			...
			if($this->form_validation->run()==true){
				...
				if(!empty($data['dusun_id']) || $data['dusun_id'] != ''){
					$dataArrayHeader[$datadusun['dusun_id']] = $datadusun;
				} else {
					$dataArrayHeader = $datadusun;
				}
				...
				$this->session->set_userdata('editcoredusun-'.$unique['unique'],$dataArrayHeader);
				$sesi 	= $this->session->userdata('unique');
				$datadusun = $this->session->userdata('editCoreOffice-'.$sesi['unique']);
				
				$datadusun['user_dusun_id'] 		= '';
				$datadusun['user_id'] 				= '';
				$datadusun['kelurahan_id'] 			= '';
				$datadusun['dusun_id'] 				= '';
		...
		public function deleteEditCoreDusun(){
			...
			redirect('CoreOffice/editCoreOffice/'.$office_id);
		}
						...
						if($this->CoreOffice_model->deleteSystemUserDusun($data['user_id'])){
							foreach ($coredusun as $key => $val) {
								...
								if($this->CoreOffice_model->insertSystemUserDusun($datadusun)){
									...
									$this->session->set_userdata('message',$msg);
									redirect('CoreOffice/editCoreOffice/'.$data['office_id']);
									break;
								}
							}
						}
