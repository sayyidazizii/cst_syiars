<?php
	Class AcctSavingsCashMutationBranch extends CI_Controller{
		...
		public function filter(){
			...
			redirect('AcctSavingsCashMutationBranch');
		}
		...
		public function getAcctSavingsCashMutationBranch(){
	        ...
	        foreach ($list as $savingsaccount) {
	            ...
	            if($savingsaccount->validation == 0){
	            	$row[] = '<a href="'.base_url().'AcctSavingsCashMutationBranch/printNoteAcctSavingsCashMutationBranch/'.$savingsaccount->savings_cash_mutation_id.'" class="btn btn-info btn-xs" role="button"><span class="glyphicon glyphicon-print"></span> Kwitansi</a> 

	            		<a href="'.base_url().'AcctSavingsCashMutationBranch/validationAcctSavingsCashMutationBranch/'.$savingsaccount->savings_cash_mutation_id.'" class="btn btn-success btn-xs" role="button"><span class="glyphicon glyphicon-check"></span> Validasi</a>';
	            } else {
	            	 $row[] = '<a href="'.base_url().'AcctSavingsCashMutationBranch/printNoteAcctSavingsCashMutationBranch/'.$savingsaccount->savings_cash_mutation_id.'" class="btn btn-info btn-xs" role="button"><span class="glyphicon glyphicon-print"></span> Kwitansi</a>';
	            }
	            $data[] = $row;
		...
		public function reset_data(){
			...
			redirect('AcctSavingsCashMutationBranch/addAcctSavingsCashMutationBranch');
		}
		...
		public function getListAcctSavingsAccount(){
	        ...
	        foreach ($list as $savingsaccount) {
	            ...
	            $row[] = '<a href="'.base_url().'AcctSavingsCashMutationBranch/addAcctSavingsCashMutationBranch/'.$savingsaccount->savings_account_id.'" class="btn btn-info" role="button"><span class="glyphicon glyphicon-ok"></span> Select</a>';
	            $data[] = $row;
	        }
		...
		public function processAddAcctSavingsCashMutationBranch(){
			...
			if($this->form_validation->run()==true){
				if($this->AcctSavingsCashMutationBranch_model->insertAcctSavingsCashMutationBranch($data)){
					...
					redirect('AcctSavingsCashMutationBranch/printNoteAcctSavingsCashMutationBranch/'.$acctsavingscash_last['savings_cash_mutation_id']);
				}else{
		...
		public function printNoteAcctSavingsCashMutationBranch(){
			...
			<table cellspacing=\"0\" cellpadding=\"1\" border=\"0\">
			    <tr>
			        <td width=\"80%\"><div style=\"text-align: center; font-size:14px\">BUKTI SETORAN TUNAI</div></td>
			    </tr>
			    <tr>
		...
		public function validationAcctSavingsCashMutationBranch(){
			...
			if($this->AcctSavingsCashMutationBranch_model->validationAcctSavingsCashMutationBranch($data)){
				...
				$this->session->set_userdata('message',$msg);
				redirect('AcctSavingsCashMutationBranch/printValidationAcctSavingsCashMutationBranch/'.$savings_cash_mutation_id);
			}else{
				$msg = "<div class='alert alert-danger alert-dismissable'> 
						<button type='button' class='close' data-dismiss='alert' aria-hidden='true'></button>
