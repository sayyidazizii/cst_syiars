<?php ob_start();?>
<?php
	Class AcctProfitLossReportNew extends CI_Controller{
		public function __construct(){
			parent::__construct();
			$this->load->model('Connection_model');
			$this->load->model('MainPage_model');
			$this->load->model('AcctProfitLossReportNew_model');
		...
		public function index(){
			...
			$sesi	= $this->session->userdata('filter-AcctProfitLossReportNew');
			...
			$data['main_view']['corebranch']					= create_double($this->AcctProfitLossReportNew_model->getCoreBranch(),'branch_id','branch_name');
			$data['main_view']['acctprofitlossreport_top']		= $this->AcctProfitLossReportNew_model->getAcctProfitLossReportNew_Top();

			$data['main_view']['acctprofitlossreport_bottom']	= $this->AcctProfitLossReportNew_model->getAcctProfitLossReportNew_Bottom();
			...
			$data['main_view']['content']						= 'AcctProfitLossReport/AcctProfitLossReportNew_view';
		...
		public function filter(){
			...
			$this->session->set_userdata('filter-AcctProfitLossReportNew',$data);
		...
		public function processPrinting(){
			...
			$data = $this->session->userdata('filter-AcctProfitLossReportNew');
			if(!is_array($data)){
			...
			$preference_company 			= $this->AcctProfitLossReportNew_model->getPreferenceCompany();

			$acctprofitlossreport_top		= $this->AcctProfitLossReportNew_model->getAcctProfitLossReportNew_Top();

			$acctprofitlossreport_bottom	= $this->AcctProfitLossReportNew_model->getAcctProfitLossReportNew_Bottom();

			$branch_name 					= $this->AcctProfitLossReportNew_model->getBranchName($data['branch_id']);
		        			...
		        			<table id=\"items\" width=\"100%\" cellspacing=\"1\" cellpadding=\"2\" border=\"0\">";	
			        			...
			        			foreach ($acctprofitlossreport_top as $keyTop => $valTop) {
									...
									if($valTop['report_type']	== 3){
										$account_subtotal 	= $this->AcctProfitLossReportNew_model->getAccountAmount($valTop['account_id'], $data['month_period'], $data['year_period'], $data['profit_loss_report_type'], $data['branch_id']);
			        		...
			        		<table id=\"items\" width=\"100%\" cellspacing=\"1\" cellpadding=\"2\" border=\"0\">";		
			        			...
			        			foreach ($acctprofitlossreport_bottom as $keyBottom => $valBottom) {
									...
									if($valBottom['report_type']	== 3){
										$account_subtotal 	= $this->AcctProfitLossReportNew_model->getAccountAmount($valBottom['account_id'], $data['month_period'], $data['year_period'], $data['profit_loss_report_type'], $data['branch_id']);
			...
			$tblFooter = "
			    ...
			    <tr>
			    	...
			    	<td style=\"border:1px black solid;\">
			    		<table id=\"items\" width=\"100%\" cellspacing=\"1\" cellpadding=\"2\" border=\"0\">
			    			<tr>
								<td style=\"width: 75%\"><div style=\"font-weight:bold;font-size:14px\">SHU</div></td>
			...
			<table cellspacing=\"0\" cellpadding=\"1\" border=\"0\" width=\"100%\">
			    <tr>
			        ...
			        <td width=\"40%\"><div style=\"text-align: center;font-size:12px\">".$this->AcctProfitLossReportNew_model->getBranchCity($data['branch_id']).", ".date('d-m-Y')."</div></td>
			    </tr>
			    <tr>
			    ...
			    <tr>
			        ...
			        <td width=\"20%\"><div style=\"text-align: center;\"></div></td>
			        <td width=\"40%\"><div style=\"text-align: center;font-size:12px;font-weight:bold\">".strtoupper($this->AcctProfitLossReportNew_model->getBranchManager($data['branch_id']))."</div></td>
			    </tr>				
			</table>";
		...
		public function exportAcctProfitLossReportNew(){
			$auth = $this->session->userdata('auth');
			$data = $this->session->userdata('filter-AcctProfitLossReportNew');
			if(!is_array($data)){
				$data['month_period']				= date('m');
			...
			if(!empty($acctprofitlossreport_top && $acctprofitlossreport_bottom)){
				...
				foreach($acctprofitlossreport_top as $keyTop => $valTop){
					if(is_numeric($keyTop)){
						...
						if($valTop['report_type']	== 3){
							$accountamount 		= $this->AcctProfitLossReportNew_model->getAccountAmount($valTop['account_id'], $data['month_period'], $data['year_period'], $data['profit_loss_report_type'], $data['branch_id']);
							...
							$this->excel->getActiveSheet()->setCellValue('C'.$j, $report_tab.$accountamount);

							$account_amount[$valTop['report_no']] = $accountamount;
						}
						...
						if($valTop['report_type'] == 6){
							if(!empty($valTop['report_formula']) && !empty($valTop['report_operator'])){
								...
								//print_r($valTop['account_name']);exit;
								$this->excel->getActiveSheet()->setCellValue('B'.$j, $report_tab.$valTop['account_name']);
				...
				foreach($acctprofitlossreport_bottom as $keyBottom => $valBottom){
					if(is_numeric($keyTop)){
						...
						if($valBottom['report_type']	== 3){
							$accountamount 		= $this->AcctProfitLossReportNew_model->getAccountAmount($valBottom['account_id'], $data['month_period'], $data['year_period'], $data['profit_loss_report_type'], $data['branch_id']);



							$account_subtotal 	= ABS(($accountamount['account_in_amount'] - $accountamount['account_out_amount']));

							//print_r($account_subtotal);exit;

							$this->excel->getActiveSheet()->setCellValue('B'.$j, $report_tab.$valBottom['account_name']);
							$this->excel->getActiveSheet()->setCellValue('C'.$j, $report_tab.$accountamount);

							$account_amount[$valBottom['report_no']] = $accountamount;
						}
				...
				$this->excel->getActiveSheet()->setCellValue('C'.$i, $this->AcctProfitLossReportNew_model->getBranchCity($data['branch_id']).", ".date('d-m-Y'));
				...
				$this->excel->getActiveSheet()->setCellValue('C'.$l, strtoupper($this->AcctProfitLossReportNew_model->getBranchManager($data['branch_id'])));
