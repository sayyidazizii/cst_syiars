...
<?php
	Class AcctConsolidationReport extends CI_Controller{
		...
		public function processPrintingAcctBalanceSheet(){
			...
			$tblHeader = "
			<table id=\"items\" width=\"100%\" cellspacing=\"1\" cellpadding=\"2\" style=\"border: 1px solid black;\">			        
			    <tr>";
			        $tblheader_left = "
			        	<td style=\"width: 50%;border-right: 1px solid black;border-bottom: 1px solid black;\">	
			        		<table id=\"items\" width=\"100%\" cellspacing=\"1\" cellpadding=\"2\" border=\"0\">";	
			        			...
			        			<tr>
									<td colspan=\"2\" style=\"width: 100%;border-bottom: 1px solid black;\"><div style=\"font-weight:bold\"> AKTIVA </div></td>
								</tr>";
			        			foreach ($acctbalancesheetreport_left as $keyLeft => $valLeft) {
									...
									if($valLeft['report_type1']	== 3){
										...
										$acctaccountlist 	= $this->AcctConsolidationReport_model->getAcctAccount_List($length, $valLeft['account_code1']);
										...
										if(!empty($acctaccountlist)){
											foreach ($acctaccountlist as $ka => $va){
												$last_balance 	= $this->AcctConsolidationReport_model->getLastBalance($va['account_id'], $sesi['consolidation_report'], $sesi['branch_id'], $last_month, $last_year);		
			        ...
			        $tblheader_right = "
			        	<td style=\"width: 50%;border-bottom: 1px solid black;\">	
			        		<table id=\"items\" width=\"100%\" cellspacing=\"1\" cellpadding=\"2\" border=\"0\">";		
			        			$tblitem_right = "
			        			<tr>
									<td colspan=\"2\" style=\"width: 100%;border-bottom: 1px solid black;\"><div style=\"font-weight:bold\"> PASIVA </div></td>
								</tr>";
			        			foreach ($acctbalancesheetreport_right as $keyRight => $valRight) {
									...
									if($valRight['report_type2']	== 3){
										...
										$acctaccountlist 	= $this->AcctConsolidationReport_model->getAcctAccount_List($length, $valRight['account_code2']);
									...
									if($valRight['report_type2']	== 8){
										...
										$sahu_tahun_lalu = $this->AcctConsolidationReport_model->getSHUTahunLalu($sesi['branch_id'], $month, $year);

										if(empty($sahu_tahun_lalu)){
											$sahu_tahun_lalu = 0;
										}

										$pembagian_shu_tahun_lalu	= $this->AcctConsolidationReport_model->getPembagianSHUTahunLalu($sesi['branch_id'], $lastdate);
									...
									if($valRight['report_type2'] == 7){
										$profit_loss = $this->AcctConsolidationReport_model->getProfitLossAmount($sesi['branch_id'], $month, $year);
			...
			$tblFooter = "
			    ...
			    <tr>
			    	<td style=\"width: 50%\">
			    		<table id=\"items\" width=\"100%\" cellspacing=\"1\" cellpadding=\"2\" border=\"0\">
			    			<tr>
								<td style=\"width: 63%\"><div style=\"font-weight:".$report_bold1.";font-size:13px\">".$report_tab1."".$valLeft['account_name1']."</div></td>
								<td style=\"width: 37%; text-align:right;\"><div style=\"font-weight:".$report_bold1."; font-size:13px\">".number_format($total_account_amount1, 2)."</div></td>
							</tr>
			    		</table>
			    	</td>
			    	<td style=\"width: 50%\">
			    		<table id=\"items\" width=\"100%\" cellspacing=\"1\" cellpadding=\"2\" border=\"0\">
			    			<tr>
								<td style=\"width: 63%\"><div style=\"font-weight:".$report_bold2.";font-size:13px\">".$report_tab2."".$valRight['account_name2']."</div></td>
								<td style=\"width: 37%; text-align:right;\"><div style=\"font-weight:".$report_bold2."; font-size:13px\">".number_format($total_account_amount2, 2)."</div></td>
							</tr>
			    		</table>
			    	</td>
			    </tr>
			...
			<table cellspacing=\"0\" cellpadding=\"1\" border=\"0\" width=\"100%\">
			    <tr>
			        ...
			        <td width=\"40%\"><div style=\"text-align: center;font-size:12px\">".$this->AcctConsolidationReport_model->getBranchCity($sesi['branch_id']).", ".date('d-m-Y')."</div></td>
			    </tr>
			    <tr>
			    ...
			    <tr>
			        ...
			        <td width=\"20%\"><div style=\"text-align: center;\"></div></td>
			        <td width=\"40%\"><div style=\"text-align: center;font-size:12px;font-weight:bold\">".strtoupper($this->AcctConsolidationReport_model->getBranchManager($sesi['branch_id']))."</div></td>
			    </tr>				
			</table>";
			...
			if(!empty($acctbalancesheetreport_left && $acctbalancesheetreport_right)){
				...
				foreach($acctbalancesheetreport_left as $keyLeft =>$valLeft){
					if(is_numeric($keyLeft)){
						...
						if($valLeft['report_type1']	== 3){
							$last_balance1 = $this->AcctConsolidationReport_model->getLastBalance($valLeft['account_id1'], $sesi['consolidation_report'], $sesi['branch_id'], $sesi['month_period'], $sesi['year_period']);		
				...
				foreach($acctbalancesheetreport_right as $keyRight =>$valRight){
					if(is_numeric($keyRight)){
						...
						if($valRight['report_type2']	== 3){
							$last_balance2 = $this->AcctConsolidationReport_model->getLastBalance($valRight['account_id2'], $sesi['consolidation_report'], $sesi['branch_id'], $sesi['month_period'], $sesi['year_period']);		
		...
		public function processPrintingAcctProfitLoss(){
			...
			// 		$data['consolidation_report']	= 9;
			// 		$data['branch_id']				= $auth['branch_id'];
			// 	}
			// } else {
			// 	$data['month_period'] 			= date('m');
			// 	$data['year_period']			= date('Y');
			// 	$data['consolidation_report']	= 0;
			// 	$data['branch_id']				= $auth['branch_id'];
			// }
		        			...
		        			<table id=\"items\" width=\"100%\" cellspacing=\"1\" cellpadding=\"2\" border=\"0\">";	
			        			...
			        			foreach ($acctprofitlossreport_top as $keyTop => $valTop) {
									...
									if($valTop['report_type']	== 3){
										...
										$account_subtotal 	= $this->AcctConsolidationReport_model->getAccountAmount($valTop['account_id'], $data['month_period'], $data['year_period'], $data['account_comparation_report_type'], $data['branch_id']);
			        		...
			        		<table id=\"items\" width=\"100%\" cellspacing=\"1\" cellpadding=\"2\" border=\"0\">";		
			        			...
			        			foreach ($acctprofitlossreport_bottom as $keyBottom => $valBottom) {
									...
									if($valBottom['report_type']	== 3){
										...
										$account_subtotal 	= $this->AcctConsolidationReport_model->getAccountAmount($valBottom['account_id'], $data['month_period'], $data['year_period'], $data['account_comparation_report_type'], $data['branch_id']);
			...
			<table cellspacing=\"0\" cellpadding=\"1\" border=\"0\" width=\"100%\">
			    <tr>
			    	<td width=\"40%\"><div style=\"text-align: center;\"></div></td>
			        <td width=\"20%\"><div style=\"text-align: center;\"></div></td>
			        <td width=\"40%\"><div style=\"text-align: center;font-size:12px\">".$this->AcctConsolidationReport_model->getBranchCity($data['branch_id']).", ".date('d-m-Y')."</div></td>
			    </tr>
			    <tr>
			        <td width=\"40%\" height=\"90px\"><div style=\"text-align: center;font-size:12px\">Yang Melaporkan</div></td>
			        <td width=\"20%\"><div style=\"text-align: center;\"></div></td>
			    ...
			    <tr>
			        <td width=\"40%\"><div style=\"text-align: center;font-size:12px;font-weight:bold\">ADMIN</div></td>
			        <td width=\"20%\"><div style=\"text-align: center;\"></div></td>
			        <td width=\"40%\"><div style=\"text-align: center;font-size:12px;font-weight:bold\">".strtoupper($this->AcctConsolidationReport_model->getBranchManager($data['branch_id']))."</div></td>
			    </tr>				
			</table>";
			...
			if(!empty($acctprofitlossreport_top && $acctprofitlossreport_bottom)){
				...
				foreach($acctprofitlossreport_top as $keyTop => $valTop){
					if(is_numeric($keyTop)){
						...
						if($valTop['report_type']	== 3){
							$accountamount 		= $this->AcctConsolidationReport_model->getAccountAmount($valTop['account_id'], $data['month_period'], $data['year_period'], $data['account_comparation_report_type'], $data['branch_id']);
				...
				foreach($acctprofitlossreport_bottom as $keyBottom => $valBottom){
					if(is_numeric($keyTop)){
						...
						if($valBottom['report_type']	== 3){
							$accountamount 		= $this->AcctConsolidationReport_model->getAccountAmount($valBottom['account_id'], $data['month_period'], $data['year_period'], $data['account_comparation_report_type'], $data['branch_id']);
