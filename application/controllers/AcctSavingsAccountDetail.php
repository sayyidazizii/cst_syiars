<?php
	Class AcctSavingsAccountDetail extends CI_Controller{
		...
		public function filter(){
			...
			redirect('AcctSavingsAccountDetail/showdetail/'.$data['savings_account_id']);
		}
		...
		public function getListAcctSavingsAccount(){
	        ...
	        foreach ($list as $savingsaccount) {
	            ...
	            $row[] = '<a href="'.base_url().'AcctSavingsAccountDetail/showdetail/'.$savingsaccount->savings_account_id.'" class="btn btn-info" role="button"><span class="glyphicon glyphicon-ok"></span> Select</a>';
	            $data[] = $row;
	        }
