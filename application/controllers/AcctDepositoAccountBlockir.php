			...
			if($this->form_validation->run()==true){
				if($deposito_account_blockir_status <> $data['deposito_account_blockir_status']){
					if($this->AcctDepositoAccountBlockir_model->insertAcctDepositoAccountBlockir($data)){
						...
						$auth = $this->session->userdata('auth');
						$this->fungsi->set_log($auth['user_id'], $auth['username'],'1003','Application.AcctDepositoAccountBlockir.processAddAcctDepositoAccountBlockir',$auth['user_id'],'Add New Deposito Account Blockir');
						...
						redirect('AcctDepositoAccountBlockir');
					}else{
	 ...
	 //        echo json_encode($output);
		...
		public function processAddAcctDepositoAccountUnBlockir(){
			...
			if($this->AcctDepositoAccountBlockir_model->updateAcctDepositoAccountBlockir($data)){
				...
				$auth = $this->session->userdata('auth');
				$this->fungsi->set_log($auth['user_id'], $auth['username'],'1003','Application.AcctDepositoAccountBlockir.processAddAcctDepositoAccountBlockir',$auth['user_id'],'Add Deposito Account UnBlockir');
				$msg = "<div class='alert alert-success alert-dismissable'>  
						<button type='button' class='close' data-dismiss='alert' aria-hidden='true'></button>					
							UnBlockir Berhasil
						</div> ";
				...
				$this->session->unset_userdata('addAcctDepositoAccountBlockir-'.$unique['unique']);
				$this->session->set_userdata('message',$msg);
				redirect('AcctDepositoAccountBlockir/unBlockirAcctDepositoAccount');
			}else{
				$this->session->set_userdata('addcoremember',$data);
				$msg = "<div class='alert alert-danger alert-dismissable'>
						<button type='button' class='close' data-dismiss='alert' aria-hidden='true'></button>					
		...
		// 	$this->session->unset_userdata('addAcctDepositoAccountBlockir-'.$unique['unique']);
		// 	redirect('AcctDepositoAccountBlockir/addAcctDepositoAccountBlockir');
		// }
	}
?>