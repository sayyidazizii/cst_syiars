<?php
	Class AcctSavingsCashMutation extends CI_Controller{
		...
		public function filter(){
			...
			redirect('AcctSavingsCashMutation');
		}
		...
		public function getAcctSavingsCashMutation(){
			...
			foreach ($list as $savingsaccount) {
				...
				if($savingsaccount->validation == 0){
					$row[] = '<a href="'.base_url().'AcctSavingsCashMutation/printNoteAcctSavingsCashMutation/'.$savingsaccount->savings_cash_mutation_id.'" class="btn btn-info btn-xs" role="button"><span class="glyphicon glyphicon-print"></span> Kwitansi</a> 

						<a href="'.base_url().'AcctSavingsCashMutation/validationAcctSavingsCashMutation/'.$savingsaccount->savings_cash_mutation_id.'" class="btn btn-success btn-xs" role="button"><span class="glyphicon glyphicon-check"></span> Validasi</a>';
				} else {
					$row[] = '<a href="'.base_url().'AcctSavingsCashMutation/printNoteAcctSavingsCashMutation/'.$savingsaccount->savings_cash_mutation_id.'" class="btn btn-info btn-xs" role="button"><span class="glyphicon glyphicon-print"></span> Kwitansi</a>';
				}
				$data[] = $row;
		...
		public function reset_data(){
			...
			redirect('AcctSavingsCashMutation/addAcctSavingsCashMutation');
		}
		...
		public function getListAcctSavingsAccount(){
			...
			foreach ($list as $savingsaccount) {
				...
				$row[] = '<a href="'.base_url().'AcctSavingsCashMutation/addAcctSavingsCashMutation/'.$savingsaccount->savings_account_id.'" class="btn btn-info" role="button"><span class="glyphicon glyphicon-ok"></span> Select</a>';
				$data[] = $row;
			}
		...
		public function addAcctSavingsCashMutation(){
			...
			$savingsaccount = $this->AcctSavingsCashMutation_model->getAcctSavingsAccount_Detail($savings_account_id);
			...
			if($savingsaccount['blocked'] == 0){
				...
				$data['main_view']['acctmutation']				= create_double($this->AcctSavingsCashMutation_model->getAcctMutationBlocked(),'mutation_id', 'mutation_name');
			}
		...
		public function processAddAcctSavingsCashMutation(){
			...
			$data = array(
				...
				'savings_cash_mutation_date'				=> tgltodb($this->input->post('savings_cash_mutation_date', true)),
			...
			if($this->form_validation->run()==true){
				if($savings_cash_mutation_token->num_rows()==0){
					if($this->AcctSavingsCashMutation_model->insertAcctSavingsCashMutation($data)){
						...
						} else if($data['mutation_id'] == $preferencecompany['cash_withdrawal_id']){
							$account_id 						= $this->AcctSavingsCashMutation_model->getAccountID($data['savings_id']);
						...
						redirect('AcctSavingsCashMutation/printNoteAcctSavingsCashMutation/'.$acctsavingscash_last['savings_cash_mutation_id']);
					}else{
		...
		public function validationAcctSavingsCashMutation(){
			...
			if($this->AcctSavingsCashMutation_model->validationAcctSavingsCashMutation($data)){
				...
				$this->session->set_userdata('message',$msg);
				redirect('AcctSavingsCashMutation/printValidationAcctSavingsCashMutation/'.$savings_cash_mutation_id);
			}else{
				$msg = "<div class='alert alert-danger alert-dismissable'> 
						<button type='button' class='close' data-dismiss='alert' aria-hidden='true'></button>
