...
<script>
	...
	mappia = "	<?php 
					$site_url = 'AcctJournalVoucher/addAcctJournalVoucher';
	...
	function function_elements_add(name, value){
		$.ajax({
				type: "POST",
				url : "<?php echo site_url('AcctJournalVoucher/function_elements_add');?>",
	...
	function reset_data(){
		document.location = base_url+"AcctJournalVoucher/reset_data";
	}
	...
	function processAddArrayAcctjournalVoucher(){
		...
		var journal_voucher_description_item		= document.getElementById("journal_voucher_description").value;
			...
			$('#onspinspinwarehouse').css('display', 'table-row');
			  $.ajax({
			  type: "POST",
			  url : "<?php echo site_url('AcctJournalVoucher/processAddArrayAcctjournalVoucher');?>",
			  data: {
...
<?php echo form_open('AcctJournalVoucher/processAddAcctJournalVoucher',array('id' => 'myform', 'class' => 'horizontal-form')); ?>
...
<div class="page-bar">
	<ul class="page-breadcrumb">
		...
		<li>
			<a href="<?php echo base_url();?>AcctJournalVoucher/addAcctJournalVoucher">
				Tambah Jurnal Umum 
			</a>
		</li>
	</ul>
							...
							<div class="col-md-5">
								<table width="100%">
									<tr>
										...
										<td width="5%">:</td>
										<td width="60%">
											<input class="easyui-datebox" data-options="formatter:myformatter,parser:myparser" type="text" name="journal_voucher_date" id="journal_voucher_date" onChange="function_elements_add(this.name, this.value);" value="<?php echo tgltoview($data['journal_voucher_date']);?>" style="width: 60%"/><input class="easyui-textbox" type="hidden" name="journal_voucher_token" id="journal_voucher_token" value="<?php echo $token;?>"/>
										</td>
									</tr>
									<tr>
									...
									<?php if($auth['branch_status'] == 1){ ?>
										<tr>
											...
											<td width="5%">:</td>
											<td width="60%">
												<?php echo form_dropdown('branch_id', $corebranch,set_value('branch_id',$data['branch_id']),'id="branch_id" class="easyui-combobox" style="width:100%"');?>
											</td>
										</tr>
									<?php } else { ?>
										<input type="hidden"  name="branch_id" id="branch_id" placeholder="Cabang" onChange="function_elements_add(this.name, this.value);" value="<?php echo $auth['branch_id']; ?>">
									<?php } ?>
										...
										if(empty($acctjournalvoucheritem)){
											...
											foreach ($acctjournalvoucheritem as $key => $val) {
												...
												<tr>
													...
													echo"
														...
														$debet = $debet + $val['journal_voucher_amount'];
													} else {
														echo"
														<td></td>
														<td style='text-align:right'>".number_format($val['journal_voucher_amount'], 2)."</td>";

														$kredit = $kredit + $val['journal_voucher_amount'];
													}
													echo"
												</tr>
												";
											...
											echo "
													<tr>
														...
														<td style='text-align:right;font-weight: bold'> ".number_format($kredit, 2)." </td>
													</tr>
												";
										}
									?>
		...
		<div class="modal-content">
			...
			<div class="modal-body">
				...
				<div class="row">
					<div class="col-md-12">							
						<div class="form-group form-md-line-input">
							<input type="text"  name="branch" id="branch" class="form-control" readonly value="<?php echo $this->AcctJournalVoucher_model->getBranchName($data['branch_id']); ?>" >
