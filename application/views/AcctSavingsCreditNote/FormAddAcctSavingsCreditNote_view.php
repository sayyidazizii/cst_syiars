...
<script>
	...
	function function_elements_add(name, value){
		$.ajax({
				type: "POST",
				url : "<?php echo site_url('AcctSavingsCreditNote/function_elements_add');?>",
	...
	function reset_data(){
		document.location = base_url+"AcctSavingsCreditNote/reset_data";
	}
	...
	$(document).ready(function(){
		$('#savings_credit_note_amount_view').textbox({
			onChange: function(value){
				...
				if(loop == 1){
					...
					$('#savings_credit_note_amount').textbox('setValue', value);
					$('#savings_credit_note_amount_view').textbox('setValue', tampil);
...
<?php echo form_open('AcctSavingsCreditNote/processAddAcctSavingsCreditNote',array('id' => 'myform', 'class' => 'horizontal-form')); ?>
<?php
	$sesi 	= $this->session->userdata('unique');
	$data 	= $this->session->userdata('addacctsavingscreditnote-'.$sesi['unique']);
	$token 	= $this->session->userdata('acctsavingscreditnotetoken-'.$sesi['unique']);
...
<div class="page-bar">
	<ul class="page-breadcrumb">
		...
		<li>
			<a href="<?php echo base_url();?>AcctSavingsCreditNote">
				Daftar Mutasi Simpanan Non Tunai - Nota Kredit
			</a>
			<i class="fa fa-angle-right"></i>
		</li>
		<li>
			<a href="<?php echo base_url();?>AcctSavingsCreditNote/addAcctSavingsCreditNote">
				Tambah Mutasi Simpanan Non Tunai - Nota Kredit
			</a>
		</li>
	</ul>
</div>
		...
		<div class="portlet"> 
			 <div class="portlet box blue">
				<div class="portlet-title">
					...
					<div class="actions">
						<a href="<?php echo base_url();?>AcctSavingsCreditNote" class="btn btn-default btn-sm">
							...
							<div class="col-md-5">
								<table width="100%">
									<tr>
										...
										<td width="5%">:</td>
										<td width="60%"><input type="text" class="easyui-textbox" style="width: 70%" name="savings_credit_note_date" id="savings_credit_note_date" autocomplete="off" value="<?php echo date('d-m-Y'); ?>" readonly/>
										</td>
									</tr>
									...
									<input type="hidden" class="easyui-textbox" name="savings_credit_note_token" id="savings_credit_note_token" autocomplete="off" value="<?php echo $token; ?>" />

									<tr>
										<td width="35%">No. Perkiraan Debet</td>
										<td width="5%">:</td>
									...
									<tr>
										...
										<td width="5%">:</td>
										<td width="60%">
											<input type="text" class="easyui-textbox" style="width: 100%" name="savings_credit_note_amount_view" id="savings_credit_note_amount_view" autocomplete="off" value="<?php echo set_value('savings_credit_note_amount',$data['savings_credit_note_amount']);?>"/>
											<input type="hidden" class="easyui-textbox" name="savings_credit_note_amount" id="savings_credit_note_amount" autocomplete="off" value="<?php echo set_value('savings_credit_note_amount',$data['savings_credit_note_amount']);?>"/>
										</td>
									</tr>
...
$(document).ready(function() {
    ...
    table = $('#myDataTable').DataTable({ 
        ...
        "ajax": {
            "url": "<?php echo site_url('AcctSavingsCreditNote/getListAcctSavingsAccount')?>",
            "type": "POST"
        },
        "columnDefs": [
        { 
