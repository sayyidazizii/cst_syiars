...
<script>
	...
	mappia 		= "<?php echo site_url('CoreOffice/editCoreOffice/'.$this->uri->segment(3)); ?>";
	function ulang(){
	...
	function function_elements_edit(name, value){
		$.ajax({
				type: "POST",
				url : "<?php echo site_url('CoreOffice/function_elements_edit');?>",
	...
	function editCoreDusun(){
		...
		$('#onspinspinwarehouse').css('display', 'table-row');
		  $.ajax({
		  type: "POST",
		  url : "<?php echo site_url('CoreOffice/editCoreDusun');?>",
		  data: {
		  			'city_id'			: city_id,
...
<?php echo form_open('CoreOffice/processEditCoreOffice',array('id' => 'myform', 'class' => 'horizontal-form')); 
...
<div class="page-bar">
	<ul class="page-breadcrumb">
		...
		<li>
			<a href="<?php echo base_url();?>CoreOffice/editCoreOffice/"<?php $this->uri->segment(3); ?>>
								...
								foreach ($datadusun as $key => $val) {
									echo "
										<tr>
											...
											<td><a href='".$this->config->item('base_url').'CoreOffice/deleteEditCoreDusun/'.$coreoffice['office_id'].'/'.$val['dusun_id']."' class='btn default btn-xs red', onClick='javascript:return confirm(\"apakah yakin ingin dihapus ?\")'>
