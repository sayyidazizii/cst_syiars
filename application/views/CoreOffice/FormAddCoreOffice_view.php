...
<script>
	...
	mappia 		= "<?php echo site_url('CoreOffice/addCoreOffice'); ?>";
	function ulang(){
	...
	function function_elements_add(name, value){
		$.ajax({
				type: "POST",
				url : "<?php echo site_url('CoreOffice/function_elements_add');?>",
		...
		$('#city_id').combobox({
			onChange: function(value){
      		...
      		alert(city_id);*/
	            ...
	            $.ajax({
	               type : "POST",
	               url  : "<?php echo base_url(); ?>CoreOffice/getCoreKecamatan/"+value,
	               data : {value: value},
		...
		$('#kecamatan_id').combobox({
			onChange: function(value){
      		...
      		alert(city_id);*/
	            ...
	            $.ajax({
	               type : "POST",
	               url  : "<?php echo base_url(); ?>CoreOffice/getCoreKelurahan/"+value,
	               data : {value: value},
	               success: function(data){
	               /*	alert(data);*/
		...
		$('#kelurahan_id').combobox({
			onChange: function(value){
      		...
      		alert(city_id);*/
	            ...
	            $.ajax({
	               type : "POST",
	               url  : "<?php echo base_url(); ?>CoreOffice/getCoreDusun/"+value,
	               data : {value: value},
	               success: function(data){
	               /*	alert(data);*/
					 	$('#dusun_id').combobox({
	...
	function addCoreDusun(){
		...
		var dusun_id 			= document.getElementById("dusun_id").value;
		...
		$('#onspinspinwarehouse').css('display', 'table-row');
		  $.ajax({
		  type: "POST",
		  url : "<?php echo site_url('CoreOffice/addCoreDusun');?>",
		  data: {
		  			'city_id'			: city_id,
...
<?php echo form_open('CoreOffice/processAddCoreOffice',array('id' => 'myform', 'class' => 'horizontal-form')); ?>
<?php
...
<div class="page-bar">
	<ul class="page-breadcrumb">
		...
		<li>
			<a href="<?php echo base_url();?>CoreOffice/addCoreOffice">
							...
							<div class="col-md-12">
								<table width="100%">
									...
									<tr>
										...
										<td width="40%"><input type="text" class="easyui-textbox" name="office_name" id="office_name" value="<?php echo $data['office_name']; ?>" style="width:100%"/></td>
										<td width="40%"><?php echo form_dropdown('branch_id', $corebranch, set_value('branch_id',$data['branch_id']),'id="branch_id" class="easyui-combobox" style="width:100%"');?></td>
									</tr>
								</table>
							</div>
						</div>
								...
								foreach ($datadusun as $key => $val) {
									echo "
										<tr>
											...
											<td>".$this->CoreOffice_model->getDusunName($val['dusun_id'])."</td>
											<td><a href='".$this->config->item('base_url').'CoreOffice/deleteCoreDusun/'.$val['record_id']."' class='btn default btn-xs red', onClick='javascript:return confirm(\"apakah yakin ingin dihapus ?\")'>
