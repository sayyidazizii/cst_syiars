	...
	<script>
		...
		function function_elements_add(name, value){
			$.ajax({
					type: "POST",
					url : "<?php echo site_url('AcctCreditAccount/function_elements_add');?>",
		...
		function reset_data(){
			document.location = base_url+"AcctCreditAccount/reset_data";
		}
		...
		$(document).ready(function(){
			$('#credit_id').combobox({
				onChange: function(value){
					...
					/* $.post(base_url + 'AcctCreditAccount/getCreditsAccountSerial',
	...
	//                	$('#credit_account_serial').textbox('setValue',obj["credit_account_serial"]);
		...
		$(document).ready(function(){		
			...
			/*$('#credit_account_margin').textbox({
				onChange: function(value){
					marginhitung();
				}
			});

			$('#credit_account_margin_rate').textbox({
				onChange: function(value){
					marginhitung();
				}
			});*/

			// $('#credit_account_payment_amount_net').textbox({
			// 	onChange: function(value){
			// 		margincalc();
			// 	}
			// });

			// $('#credit_account_payment_amount_margin').textbox({
			// 	onChange: function(value){
			// 		margincalc();
			// 	}
			// });
						...
						<table style="width: 100%;" border="0" padding:"0">
								<tbody>
									...
									<tr>
										...
										<td><input type="text" class="form-control" name="savings_account_no" id="savings_account_no" autocomplete="off" readonly value="<?php echo $acctsavingsaccount['savings_account_no'];?>" tabindex="-1"/>
										<input type="hidden" class="form-control" name="savings_account_id" id="savings_account_id" autocomplete="off" readonly value="<?php echo $acctsavingsaccount['savings_account_id'];?>"/></td>
										<td><a href="#" role="button" class="btn btn-info btn-sm" data-toggle="modal" data-target="#savingslist">Cari No. Rek</a></td>
										<td>Agunan</td>
										<td>:</td>
	...
	<div id="savingslist" class="modal fade" role="dialog">
	<div class="modal-dialog modal-lg">
		...
		<div class="modal-header">
			<button type="button" class="close" data-dismiss="modal">&times;</button>
			<h4 class="modal-title">No. Rekening List</h4>
		</div>
		<div class="modal-body">
	...
	$(document).ready(function() {
		...
		table2 = $('#myDataTable2').DataTable({ 
			...
			"order": [], //Initial no order.
			"ajax": {
				"url": "<?php echo site_url('AcctCreditAccount/savingslist/'.$member_id)?>",
				"type": "POST"
			},
			"columnDefs": [
			{ 
