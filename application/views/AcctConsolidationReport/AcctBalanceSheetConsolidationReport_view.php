...
<?php echo form_open('AcctConsolidationReport/filterAcctBalanceSheet',array('id' => 'myform', 'class' => '')); ?>
<div class="row">
	<div class="col-md-12">	
											...
											<?php
												foreach ($acctbalancesheetreport_left as $key => $val) {
													...
													echo "
														...
														// 	$last_balance1 = $this->AcctConsolidationReport_model->getLastBalance($val['account_id1'], $data['consolidation_report'], $data['branch_id'], $data['month_period'], $data['year_period']);

														// 	if(empty($last_balance1)){
														// 		$last_balance1 = $this->AcctConsolidationReport_model->getLastBalance2($val['account_id1'], $data['consolidation_report'], $data['branch_id'], $data['month_period'], $data['year_period']);
														// 	}
														...
														if($val['report_type1']	== 3){
															...
															$acctaccountlist 	= $this->AcctConsolidationReport_model->getAcctAccount_List($length, $val['account_code1']);

															
															$last_balance1 = 0;
															if(!empty($acctaccountlist)){
																foreach ($acctaccountlist as $ka => $va){
																	$last_balance 	= $this->AcctConsolidationReport_model->getLastBalance($va['account_id'],$data['consolidation_report'], $data['branch_id'], $last_month, $last_year);
												...
												foreach ($acctbalancesheetreport_right as $key => $val) {
													...
													echo "
														...
														if($val['report_type2']	== 3){
															...
															$acctaccountlist 	= $this->AcctConsolidationReport_model->getAcctAccount_List($length, $val['account_code2']);

															if ($val['account_code2'] == '270'){
																print_r("acctaccountlist ");
																print_r($acctaccountlist);
																print_r("<BR> ");
															}
													...
													echo "
														...
														if($val['report_type2']	== 8){
															$sahu_tahun_lalu = $this->AcctConsolidationReport_model->getSHUTahunLalu($data['branch_id'], $month, $year);

															if(empty($sahu_tahun_lalu)){
																$sahu_tahun_lalu = 0;
															}

															$pembagian_shu_tahun_lalu	= $this->AcctConsolidationReport_model->getPembagianSHUTahunLalu($data['branch_id'], $lastdate);


															$sisa_shu_tahun_lalu 		= $sahu_tahun_lalu - $pembagian_shu_tahun_lalu;

															print_r("sahu_tahun_lalu ");
															print_r($sahu_tahun_lalu);
															print_r("<BR> ");
															print_r("<BR> ");

															print_r("pembagian_shu_tahun_lalu ");
															print_r($pembagian_shu_tahun_lalu);
															print_r("<BR> ");
															print_r("<BR> ");

															print_r("sisa_shu_tahun_lalu ");
															print_r($sisa_shu_tahun_lalu);
															print_r("<BR> ");
															print_r("<BR> ");
															...
															echo "
																...
																<td style='text-align:right'><div style='font-weight:".$report_bold2."'>".number_format($sisa_shu_tahun_lalu, 2)."</div></td>
															";

															$account_amount2_bottom[$val['report_no']] = $sisa_shu_tahun_lalu;
														}
													...
													echo "
														...
														if($val['report_type2']	== 7){
															$profit_loss = $this->AcctConsolidationReport_model->getProfitLossAmount($data['branch_id'], $month, $year);
