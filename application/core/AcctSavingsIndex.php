	...
	Class AcctSavingsIndex extends CI_Controller{
		...
		public function processAddAcctSavingsIndex(){
			...
			if($this->form_validation->run()==true){
				...
				// if($date == $last_date){
					...
					$daily_average_balance_accumulation = $this->AcctSavingsIndex_model->getDailyAverageBalanceAccumulation($month, $year, $auth['branch_id']);
					...
					// print_r($deposito_last_balance_accumulation); exit;

					foreach ($acctsavings as $keyS => $valS) {
						$bmt_percentage 		= 100 - $valS['savings_nisbah'];
						$daily_avreage_balance 	= $this->AcctSavingsIndex_model->getDailyAverageBalance_Savings($month, $year, $valS['savings_id'], $auth['branch_id']);

						$portion_per_savings 	= ($daily_avreage_balance / $total_accumulation) * $data['income_amount'];

						// $savings_index_amount = (($data['income_amount'] / $total_accumulation) * $valS['savings_nisbah']) / 100;
					...
					foreach ($acctdeposito as $keyD => $valD) {
						// $deposito_index_amount = (($data['income_amount'] / $total_accumulation) * $valD['deposito_interest_rate']) / 100;
		...
		public function processPrinting(){
			...
			$tbl = "
				<table cellspacing=\"0\" cellpadding=\"1\" border=\"0\" width=\"100%\">
					<tr>
						<td width=\"30%\"><div style=\"text-align: left;font-size: 12px\">TOTAL SADLO SIMP & BERJANGKA </div></td>
						<td width=\"60%\"><div style=\"text-align: left;font-size: 12px\">:  ".number_format($total['total_savings_deposito'], 2)."</div></td>
				    </tr>
				    <tr>
						<td width=\"30%\"><div style=\"text-align: left;font-size: 12px\">TOTAL SADLO RATA2 HARIAN </div></td>
