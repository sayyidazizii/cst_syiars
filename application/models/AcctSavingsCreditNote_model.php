<?php
	class AcctSavingsCreditNote_model extends CI_Model {
		var $table = "acct_savings_credit_note";
		...
		var $order = array('acct_savings_credit_note.savings_credit_note_id' => 'asc');
		...
		public function getAcctSavingsCreditNote($start_date, $end_date, $savings_account_id){
			$this->db->select('acct_savings_credit_note.savings_credit_note_id, acct_savings_credit_note.savings_account_id, acct_savings_account.savings_account_no, acct_savings_credit_note.member_id, core_member.member_name, acct_savings_credit_note.savings_id, acct_savings.savings_name, acct_savings_credit_note.savings_credit_note_date, acct_savings_credit_note.savings_credit_note_amount, acct_savings_credit_note.mutation_id, acct_mutation.mutation_name, acct_savings_credit_note.validation, acct_savings_credit_note.validation_on, acct_savings_credit_note.validation_id');
			$this->db->from('acct_savings_credit_note');
			$this->db->join('acct_mutation', 'acct_savings_credit_note.mutation_id = acct_mutation.mutation_id');
			$this->db->join('acct_savings_account', 'acct_savings_credit_note.savings_account_id = acct_savings_account.savings_account_id');
			$this->db->join('core_member', 'acct_savings_credit_note.member_id = core_member.member_id');
			$this->db->join('acct_savings', 'acct_savings_credit_note.savings_id = acct_savings.savings_id');
			$this->db->where('acct_savings_credit_note.savings_credit_note_date >=', $start_date);
			$this->db->where('acct_savings_credit_note.savings_credit_note_date <=', $end_date);
			if(!empty($savings_account_id)){
				$this->db->where('acct_savings_credit_note.savings_account_id', $savings_account_id);
			}
			$this->db->where('acct_savings_credit_note.data_state', 0);
	    ...
	    public function count_all($start_date, $end_date, $branch_id)
	    {
			...
			$this->db->where('acct_savings_credit_note.savings_credit_note_date <=', $end_date);
			if(!empty($branch_id)){
				$this->db->where('acct_savings_credit_note.branch_id', $branch_id);
			}
			$this->db->where('acct_savings_credit_note.data_state', 0);
	    ...
	    private function _get_datatables_query($start_date, $end_date, $branch_id)
	    {
	       ...
	       $this->db->from('acct_savings_credit_note');
			$this->db->join('acct_account', 'acct_savings_credit_note.account_id = acct_account.account_id');
			$this->db->join('acct_mutation', 'acct_savings_credit_note.mutation_id = acct_mutation.mutation_id');
			$this->db->join('acct_savings_account', 'acct_savings_credit_note.savings_account_id = acct_savings_account.savings_account_id');
			$this->db->join('core_member', 'acct_savings_credit_note.member_id = core_member.member_id');
			$this->db->join('acct_savings', 'acct_savings_credit_note.savings_id = acct_savings.savings_id');
		...
		public function getAcctMutationCreditNote(){
			$this->db->select('mutation_id, mutation_code, mutation_name');
			$this->db->from('acct_mutation');
			$this->db->where('mutation_id', 10);
		...
		public function insertAcctSavingsCreditNote($data){
			return $query = $this->db->insert('acct_savings_credit_note',$data);
		}

		public function getSavingsCashMutationToken($savings_credit_note_token){
			$this->db->select('savings_credit_note_token');
			$this->db->from('acct_savings_credit_note');
			$this->db->where('savings_credit_note_token', $savings_credit_note_token);
			return $this->db->get();
		}
		...
		public function getAcctSavingsCreditNote_Last($created_id){
			$this->db->select('acct_savings_credit_note.savings_credit_note_id, acct_savings_credit_note.savings_account_id, acct_savings_account.savings_account_no, acct_savings_credit_note.member_id, core_member.member_name');
			$this->db->from('acct_savings_credit_note');
			$this->db->join('acct_savings_account','acct_savings_credit_note.savings_account_id = acct_savings_account.savings_account_id');
			$this->db->join('core_member','acct_savings_credit_note.member_id = core_member.member_id');
			$this->db->where('acct_savings_credit_note.created_id', $created_id);
			$this->db->limit(1);
			$this->db->order_by('acct_savings_credit_note.savings_credit_note_id','DESC');
			$result = $this->db->get()->row_array();
			return $result;
		}
		...
		public function getAcctSavingsCreditNote_Detail($savings_credit_note_id){
			$this->db->select('acct_savings_credit_note.savings_credit_note_id, acct_savings_credit_note.savings_account_id, acct_savings_account.savings_account_no, acct_savings_credit_note.savings_id, acct_savings.savings_name, acct_savings_credit_note.mutation_id, acct_mutation.mutation_name, acct_savings_credit_note.member_id, core_member.member_name, core_member.member_address, core_member.city_id, core_member.kecamatan_id, acct_savings_credit_note.branch_id, core_branch.branch_city,  core_member.identity_id, core_member.member_identity_no, acct_savings_credit_note.savings_credit_note_date, acct_savings_credit_note.savings_credit_note_amount, acct_savings_credit_note.validation, acct_savings_credit_note.validation_on, acct_savings_credit_note.validation_id');
			$this->db->from('acct_savings_credit_note');
			$this->db->join('acct_mutation', 'acct_savings_credit_note.mutation_id = acct_mutation.mutation_id');
			$this->db->join('acct_savings_account', 'acct_savings_credit_note.savings_account_id = acct_savings_account.savings_account_id');
			$this->db->join('core_member', 'acct_savings_credit_note.member_id = core_member.member_id');
			$this->db->join('acct_savings', 'acct_savings_credit_note.savings_id = acct_savings.savings_id');
			$this->db->join('core_branch', 'acct_savings_credit_note.branch_id = core_branch.branch_id');
			$this->db->where('acct_savings_credit_note.data_state', 0);
			$this->db->where('acct_savings_credit_note.savings_credit_note_id', $savings_credit_note_id);
			return $this->db->get()->row_array();
		}
		...
		public function validationAcctSavingsCreditNote($data){
			$this->db->where("savings_credit_note_id",$data['savings_credit_note_id']);
		...
		public function voidAcctSavingsCreditNote($data){
			$this->db->where("savings_credit_note_id",$data['savings_credit_note_id']);
			$query = $this->db->update($this->table, $data);
			if($query){
				return true;
		...
		public function getAcctSavingsCreditNote_Member($member_id, $savings_credit_note_date, $data_mutation){
			$this->db->select('acct_savings_credit_note.savings_credit_note_id, acct_savings_credit_note.savings_account_id, acct_savings_account.savings_account_no, acct_savings_credit_note.savings_id, acct_savings.savings_code, acct_savings.savings_name, acct_savings_credit_note.mutation_id, acct_mutation.mutation_name, acct_savings_credit_note.member_id, core_member.member_name, core_member.member_address, core_member.city_id, core_member.kecamatan_id, acct_savings_credit_note.branch_id, core_branch.branch_city,  core_member.identity_id, core_member.member_identity_no, acct_savings_credit_note.savings_credit_note_date, acct_savings_credit_note.savings_credit_note_amount, acct_savings_credit_note.validation, acct_savings_credit_note.validation_on, acct_savings_credit_note.validation_id');
			$this->db->from('acct_savings_credit_note');
			$this->db->join('acct_mutation', 'acct_savings_credit_note.mutation_id = acct_mutation.mutation_id');
			$this->db->join('acct_savings_account', 'acct_savings_credit_note.savings_account_id = acct_savings_account.savings_account_id');
			$this->db->join('core_member', 'acct_savings_credit_note.member_id = core_member.member_id');
			...
			$this->db->join('core_branch', 'acct_savings_credit_note.branch_id = core_branch.branch_id');
			$this->db->where('acct_savings_credit_note.data_state', 0);
			$this->db->where('acct_savings_credit_note.member_id', $member_id);
			$this->db->where('acct_savings_credit_note.savings_credit_note_date', $savings_credit_note_date);
			$this->db->where_in('acct_savings_credit_note.mutation_id', $data_mutation);
			$this->db->where_in('acct_savings_credit_note.savings_credit_note_status', 1);
			$result = $this->db->get()->result_array();
			return $result;
		}
	}
