<?php
	...
	class AcctCreditAccountUtility_model extends CI_Model {
		...
		var $column_search = array('member_name','member_no','member_address'); //field yang diizin untuk pencarian 
		var $order = array('core_member.member_id' => 'asc');
		...
		public function getAcctCreditsAccount_Detail($credits_account_id){
			$this->db->select('acct_credits_account.*, core_member.member_name, core_member.member_no, core_member.member_address, core_member.province_id, core_province.province_name, core_member.city_id, core_city.city_name, core_member.kecamatan_id, core_kecamatan.kecamatan_name, core_member.member_identity, core_member.member_identity_no, acct_credits.credits_name');
			$this->db->from('acct_credits_account');
			$this->db->join('acct_credits', 'acct_credits_account.credits_id = acct_credits.credits_id');
			$this->db->join('core_member', 'acct_credits_account.member_id = core_member.member_id');
			$this->db->join('core_province', 'core_member.province_id = core_province.province_id');
		...
		public function insertAcctCreditAccountUtility($data){
		...
		public function updateAcctCreditAccountUtility($data){
	    ...
	    public function count_all_master($start_date, $end_date, $credits_id, $branch_id)
	    {
			...
			$this->db->where('acct_credits_account.credits_account_date <=', $end_date);
			if(!empty($credits_id)){
				$this->db->where('acct_credits_account.credits_id', $created_id);
			}
			if(!empty($branch_id)){
				$this->db->where('acct_credits_account.branch_id', $branch_id);
			}
