<?php
	class AcctCreditsDataMutation_model extends CI_Model {
		...
		public function getAcctCreditsAccount($credits_id){
			$this->db->select('acct_credits_account.credits_account_id, acct_credits_account.credits_account_serial, acct_credits_account.member_id, core_member.member_name, core_member.member_address, acct_credits_account.credits_account_last_number');
		...
		public function getAcctCredits(){
			$this->db->select('credits_id, credits_name');
			$this->db->from('acct_credits');
			$this->db->where('data_state', 0);
			$this->db->where('credits_status', 0);
		...
		public function getCreditsNisbah($credits_id){
			$this->db->select('credits_nisbah');
			$this->db->from('acct_credits');
			$this->db->where('credits_id', $credits_id);
			$result = $this->db->get()->row_array();
			return $result['credits_nisbah'];
		}
		...
		public function getLastAccountCreditsNo($credits_id){
			$this->db->select('credits_account_no');
			$this->db->from('acct_credits_account');
			$this->db->where('credits_id', $credits_id);
			$this->db->limit(1);
			$this->db->order_by('credits_account_id','DESC');
			$result = $this->db->get()->row_array();
			return $result['credits_account_no'];
		}
		
		
		public function getCoreMember_Detail($member_id){
			$this->db->select('core_member.member_id, core_member.branch_id, core_branch.branch_name, core_member.member_no, core_member.member_name, core_member.member_gender, core_member.member_place_of_birth, core_member.member_date_of_birth, core_member.member_address, core_member.province_id, core_province.province_name, core_member.city_id, core_city.city_name, core_member.kecamatan_id, core_kecamatan.kecamatan_name, core_member.member_phone, core_member.member_job, core_member.identity_id, core_member.member_identity_no, core_member.member_postal_code, core_member.member_mother, core_member.member_heir, core_member.member_family_relationship, core_member.member_status, core_member.member_register_date, core_member.member_principal_credits, core_member.member_special_credits, core_member.member_mandatory_credits');
