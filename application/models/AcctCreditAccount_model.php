<?php
	...
	class AcctCreditAccount_model extends CI_Model {
		...
		public function getBranchAddress($branch_id){
			$this->db->select('core_branch.branch_address');
			$this->db->from('core_branch');
			$this->db->where('core_branch.branch_id', $branch_id);
			$result = $this->db->get()->row_array();
			return $result['branch_address'];
		}

		public function getBranchNoSK($branch_id){
			$this->db->select('core_branch.branch_no_sk');
			$this->db->from('core_branch');
			$this->db->where('core_branch.branch_id', $branch_id);
			$result = $this->db->get()->row_array();
			return $result['branch_no_sk'];
		}
		...
		public function getAcctCreditsAccount_Detail($credits_account_id){
			$this->db->select('acct_credits_account.*, core_member.member_name, core_member.member_no, core_member.member_address, core_member.province_id, core_province.province_name, core_member.city_id, core_city.city_name, core_member.kecamatan_id, core_kecamatan.kecamatan_name, core_member.member_identity, core_member.member_identity_no, acct_credits.credits_name, core_branch.branch_code');
			$this->db->from('acct_credits_account');
			$this->db->join('acct_credits', 'acct_credits_account.credits_id = acct_credits.credits_id');
		...
		public function getAcctCreditsPayment_Detail($credits_account_id){
			$this->db->select('acct_credits_payment.credits_payment_date, acct_credits_payment.credits_payment_amount, acct_credits_payment.credits_payment_principal, acct_credits_payment.credits_payment_margin, acct_credits_payment.credits_principal_last_balance, acct_credits_payment.credits_margin_last_balance');
		...
		public function getLastAccountCreditsNo($branch_id, $credits_id){
			...
			$this->db->where('acct_credits_account.credits_id', $credits_id);
			$this->db->limit(1);
			$this->db->order_by('last_credits_account_serial','DESC');
			$result = $this->db->get();
			return $result;
		}
		...
		public function getAcctCreditsAccount_Last($created_id){
			...
			$this->db->join('core_member','acct_credits_account.member_id = core_member.member_id');
			$this->db->join('acct_credits','acct_credits_account.credits_id = acct_credits.credits_id');
			$this->db->where('acct_credits_account.created_id', $created_id);
			$this->db->limit(1);
			$this->db->order_by('acct_credits_account.credits_account_id','DESC');
			$result = $this->db->get()->row_array();
			return $result;
		}
		...
		private function _get_datatables_query($branch_id)
		{
			...
			// $this->db->or_where('acct_credits_account.credits_account_last_balance_margin !=', 0);
			if(!empty($branch_id)){
				$this->db->where('acct_credits_account.branch_id', $branch_id);
			}
		...
		function get_datatables_all($branch_id)
		{
			$this->_get_datatables_query_all($branch_id);
			// $this->db->join('core_member','acct_credits_account.member_id=core_member.member_id');
			if($_POST['length'] != -1)
			$this->db->limit($_POST['length'], $_POST['start']);
			$query = $this->db->get();
		...
		function count_filtered_all($branch_id)
		{
			$this->_get_datatables_query_all($branch_id);
			$query = $this->db->get();
			return $query->num_rows();
		...
		public function count_all_all($branch_id)
		{
			$this->db->from('acct_credits_account');
			$this->db->join('core_member','acct_credits_account.member_id=core_member.member_id');
			if(!empty($branch_id)){
		...
		private function _get_datatables_query_all($branch_id)
		{
			// $this->db->select('acct_credits_account.credits_account_id, acct_credits_account.credits_account_serial');
			$this->db->from('acct_credits_account');
			$this->db->join('core_member','acct_credits_account.member_id=core_member.member_id');
