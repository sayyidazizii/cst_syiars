<?php
	class AcctSavingsPrintSavingsMonitor_model extends CI_Model {
		...
		public function insertAcctSavingsSyncronizeLog($data){
			if($this->db->insert('acct_savings_syncronize_log', $data)){
				return true;
			} else {
				return false;
			}
		...
		public function getOpeningBalance($savings_account_id, $date){
			$this->db->select('opening_balance');
			$this->db->from('acct_savings_account_detail');
			$this->db->where('savings_account_id', $savings_account_id);
			$this->db->where('today_transaction_date', $date);
			$this->db->order_by('savings_account_detail_id', 'DESC');
			$this->db->limit(1);
			$result = $this->db->get()->row_array();
			return $result['opening_balance'];
		}

		public function getLastDate($savings_account_id, $start_date){
			$this->db->select_max('today_transaction_date');
			$this->db->from('acct_savings_account_detail');
			$this->db->where('savings_account_id', $savings_account_id);
		...
		public function getLastBalance($savings_account_id, $date){
