<?php
	class AcctSavingsPrintMutation_model extends CI_Model {
		...
		public function getAcctSavingsAccountDetail($savings_account_id, $start_date, $end_date){
			$this->db->select('acct_savings_account_detail.savings_account_detail_id, acct_savings_account_detail.savings_account_id, acct_savings_account.savings_account_no, acct_savings_account_detail.member_id, core_member.member_name, core_member.member_address, acct_savings_account_detail.mutation_id, acct_mutation.mutation_status, acct_mutation.mutation_code, acct_savings_account_detail.mutation_in, acct_savings_account_detail.mutation_out, acct_savings_account_detail.opening_balance, acct_savings_account_detail.last_balance, acct_savings_account_detail.savings_id, acct_savings.savings_name, acct_savings_account_detail.today_transaction_date, acct_savings_account_detail.created_id, acct_savings_account_detail.description, acct_savings_account_detail.operated_name, acct_savings_account.savings_account_last_number, acct_savings_account_detail.savings_print_status');
			...
			$this->db->where('acct_savings_account_detail.savings_print_status', 0);
			$result = $this->db->get()->result_array();
			return $result;
		}
		...
		public function updatePrintMutationStatus($data){
			$this->db->set('acct_savings_account_detail.savings_print_status', $data['savings_print_status']);
			$this->db->where('acct_savings_account_detail.savings_account_detail_id', $data['savings_account_detail_id']);
			if($this->db->update('acct_savings_account_detail')){
				$this->db->set('acct_savings_account.savings_account_last_number', $data['savings_account_last_number']);
				$this->db->where('acct_savings_account.savings_account_id', $data['savings_account_id']);
		...
		public function getSavingsAcountLastNumber($savings_account_id){
			$this->db->select('savings_account_last_number');
			$this->db->from('acct_savings_account');
			$this->db->where('savings_account_id', $savings_account_id);
			$result = $this->db->get()->row_array();
			return $result['savings_account_last_number'];
		}
	}
?>